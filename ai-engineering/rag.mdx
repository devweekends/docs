...
```python
class AgenticRAG:
    """RAG with multi-step reasoning -- decomposes complex questions
    into simpler sub-questions, answers each, then synthesizes."""
...
Choosing the right retrieval strategy is the single most impactful decision in your RAG pipeline. Here is a decision table for the three main strategies and their sub-techniques:

| Signal in Your Data | Recommended Strategy | Why |
|---------------------|---------------------|-----|
...
<AccordionGroup>
  <Accordion title="Retrieval returns irrelevant documents" icon="circle-xmark">
    ...
    **Root Cause**: Usually a chunking problem. If your chunks are too large, every chunk is "kind of about everything" and similarity scores flatten. If too small, they lack enough meaning to match well.
  ...
  <Accordion title="LLM ignores provided context" icon="ban">
    **Symptoms**: Answer doesn't use sources, makes up facts. This is the most dangerous RAG failure because it looks correct but is hallucinated.
    
    **Root Cause**: The LLM's parametric knowledge (training data) is competing with your retrieved context. If the context is poorly formatted or buried in the prompt, the model defaults to what it "knows."
