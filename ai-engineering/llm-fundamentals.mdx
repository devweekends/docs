...
```
"I love pizza" → [0.2, 0.8, -0.1, ...]  (1536 numbers)
"Pizza is my favorite food" → [0.21, 0.79, -0.11, ...]  (similar vector)
"Stock prices fell" → [-0.5, 0.1, 0.9, ...]  (different vector)
```
...
You don't need to understand the math, but knowing the intuition helps.

### Self-Attention Intuition

When processing "The cat sat on the mat", the model asks for each word:
- **"sat"**: Which other words are relevant? → "cat" (who sat?), "mat" (where?)
- **"it"** (if present): What does "it" refer to? → Probably "cat"

This is how LLMs handle long-range dependencies and context.
