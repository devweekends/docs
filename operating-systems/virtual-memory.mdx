...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Process A           Process B           Physical Memory        │
│  ┌──────────┐        ┌──────────┐        ┌──────────────┐      │
│  │ 0x0000   │        │ 0x0000   │        │ 0x00000000   │      │
│  │   Code   │───┐    │   Code   │───┐    │    Frame 0   │      │
│  │          │   │    │          │   │    ├──────────────┤      │
│  │ 0x1000   │   │    │ 0x1000   │   │    │    Frame 1   │←─A's code
│  │   Data   │───┼────│   Data   │───┼────├──────────────┤      │
│  │          │   │    │          │   │    │    Frame 2   │←─B's code
│  │ 0x2000   │   │    │ 0x2000   │   │    ├──────────────┤      │
│  │   Heap   │   │    │   Heap   │   └───→│    Frame 3   │←─B's data
│  │          │   │    │          │        ├──────────────┤      │
│  │  ...     │   └───────────────────────→│    Frame 4   │←─A's data
│  │          │                            ├──────────────┤      │
│  │ 0xFFFF   │                            │     ...      │      │
...
│   (16 bits)  │(9 bit)│(9 bit)│(9 bit)│(9 bit)│  (12 bits) │
└──────────────┴───────┴───────┴───────┴───────┴────────────┘
        │           │       │       │       │        │
        │           └───────┴───────┴───────┘        │
        │                    │                       │
   Sign extended        Page table           Offset within
   (must match           walk              4KB page (2^12)
    bit 47)

Page sizes:
- 4KB pages: 12-bit offset (most common)
- 2MB pages: 21-bit offset (huge pages)
- 1GB pages: 30-bit offset (gigantic pages)
```
...
│        ▼                                                        │
│  ┌───────────┐                                                  │
│  │    PD     │ ←── Level 2 (512 entries, each covers 2MB)     │
│  │  Table    │                                                  │
│  └─────┬─────┘                                                  │
│        │ [PD index]                                             │
│        ▼                                                        │
│  ┌───────────┐                                                  │
│  │    PT     │ ←── Level 1 (512 entries, each covers 4KB)     │
│  │  Table    │                                                  │
│  └─────┬─────┘                                                  │
│        │ [PT index]                                             │
│        ▼                                                        │
│  ┌───────────┐                                                  │
│  │  Physical │ + Offset = Physical Address                     │
│  │   Frame   │                                                  │
│  └───────────┘                                                  │
│                                                                  │
│  Memory accesses for one translation: 4 (without TLB)          │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Memory Access ──▶ MMU ──▶ Translation Fails ──▶ Exception     │
│                                       │                         │
│                                       ▼                         │
│                            ┌───────────────────┐               │
│                            │  Is address valid? │               │
...
│                        ▼                           ▼           │
│             ┌──────────────────┐         ┌─────────────────┐  │
│             │ Is page in swap? │         │ Send SIGSEGV to │  │
│             └────────┬─────────┘         │     process     │  │
│                      │                   └─────────────────┘  │
│           ┌──────────┴──────────┐                             │
│           │                     │                             │
│           ▼                     ▼                             │
│      In swap/file          First access                       │
│           │                     │                             │
│           ▼                     ▼                             │
│   ┌───────────────┐    ┌────────────────┐                    │
│   │ Read from disk│    │ Allocate frame │                    │
...
│  3. If reference bit = 1: Clear bit, advance clock             │
│  4. Repeat until finding page with bit = 0                     │
│                                                                  │
│  Reference bit set to 1 on every access (by hardware)          │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
│  ┌────────┐        ┌────────┐        ┌─────────────┐           │
│  │ VPN 0 ─┼────┐   │ VPN 0 ─┼────┐   │  Frame 100  │           │
│  │ (R/O)  │    └───│ (R/O)  │    └──→│  (shared)   │           │
│  ├────────┤        ├────────┤        ├─────────────┤           │
│  │ VPN 1 ─┼────┐   │ VPN 1 ─┼────┐   │  Frame 101  │           │
│  │ (R/O)  │    └───│ (R/O)  │    └──→│  (shared)   │           │
│  └────────┘        └────────┘        └─────────────┘           │
│                                                                  │
│  After parent writes to VPN 1:                                  │
│                                                                  │
│  Parent PT         Child PT          Physical Memory            │
│  ┌────────┐        ┌────────┐        ┌─────────────┐           │
│  │ VPN 0 ─┼────────│ VPN 0 ─┼───────→│  Frame 100  │           │
...
**Trade-offs**:
- ✅ Fewer TLB misses, better for large working sets
- ❌ More internal fragmentation
- ❌ Memory compaction overhead (for THP)
