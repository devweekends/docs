...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Block 0          Block 1         Blocks 2-N      Remaining     │
│  ┌──────────┐    ┌──────────┐    ┌──────────┐    ┌──────────┐  │
│  │Superblock│    │  Inode   │    │   Data   │    │   Data   │  │
│  │          │    │  Bitmap  │    │  Bitmap  │    │  Blocks  │  │
│  │ - Magic  │    │          │    │          │    │          │  │
│  │ - Size   │    │ 1=used   │    │ 1=used   │    │ file/dir │  │
│  │ - Counts │    │ 0=free   │    │ 0=free   │    │ contents │  │
│  └──────────┘    └──────────┘    └──────────┘    └──────────┘  │
│                                                                  │
│                  ┌──────────────────────────────┐               │
│                  │        Inode Table           │               │
...
│  │ indirect   ────┼──► ┌────────────┐                          │
│  │                │    │ ptr → data │──► Data Block 12         │
│  │                │    │ ptr → data │──► Data Block 13         │
│  │                │    │ ...        │    ...                   │
│  │                │    └────────────┘    (1024 more blocks)    │
│  │                │                                             │
│  │ double_ind ────┼──► ┌────────────┐                          │
...
│  │                │                      (1024² more blocks)   │
│  │                │                                             │
│  │ triple_ind ────┼──► (1024³ blocks, up to 4TB file)          │
│  └────────────────┘                                             │
│                                                                  │
│  Max file size with 4KB blocks:                                 │
│  12 + 1024 + 1024² + 1024³ = ~4 TB                             │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
│  HARD LINK:                                                     │
│                                                                  │
│  Directory A          Directory B          Inode 42             │
│  ┌──────────────┐    ┌──────────────┐    ┌──────────┐          │
│  │ "file.txt"   │    │ "link.txt"   │    │ nlink: 2 │          │
│  │ inode: 42  ──┼────┤ inode: 42  ──┼───►│ data...  │          │
│  └──────────────┘    └──────────────┘    └──────────┘          │
│                                                                  │
│  - Both names point to same inode                               │
│  - Delete one, other still works                                │
│  - File deleted when nlink = 0                                  │
│  - Cannot span filesystems                                      │
│                                                                  │
│  SOFT LINK (Symlink):                                           │
│                                                                  │
│  Directory           Inode 50           Inode 42                │
│  ┌──────────────┐   ┌────────────┐    ┌──────────┐            │
│  │ "symlink"    │   │ type: link │    │ nlink: 1 │            │
│  │ inode: 50  ──┼──►│ data:      │───►│ data...  │            │
│  └──────────────┘   │ "/a/file"  │    └──────────┘            │
│                     └────────────┘                              │
│                                                                  │
│  - Symlink is separate file containing path                    │
│  - Delete target: symlink becomes "dangling"                   │
│  - Can span filesystems, point to directories                  │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Directory inode (inode 2, typically root)                      │
│                                                                  │
│  Data block:                                                    │
│  ┌─────────────────────────────────────────────────────────┐   │
...
│  └───────┴─────────┴──────────┴──────┴────────────────────┘   │
│                                                                  │
│  type: 1=regular file, 2=directory, 7=symlink                   │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
│  File C: start=9, length=3                                      │
│                                                                  │
│  ✓ Fast sequential access                                       │
│  ✓ Simple metadata                                              │
│  ✗ External fragmentation                                       │
│  ✗ File growth difficult (need to relocate)                    │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
│  ┌─────────┐    ┌─────────┐    ┌─────────┐    ┌─────────┐     │
│  │ data    │    │ data    │    │ data    │    │ data    │     │
│  │ next: 4 │───►│ next: 2 │───►│ next: 7 │───►│ next:-1 │     │
│  └─────────┘    └─────────┘    └─────────┘    └─────────┘     │
│     block 0       block 4       block 2        block 7         │
│                                                                  │
│  ✓ No external fragmentation                                    │
│  ✓ Files can grow easily                                        │
│  ✗ Random access is O(n) — must traverse chain                  │
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Index block (part of inode):                                   │
│  ┌──────────────────┐                                           │
│  │ direct[0] = 4    │──────► block 4: data                     │
│  │ direct[1] = 7    │──────► block 7: data                     │
│  │ direct[2] = 2    │──────► block 2: data                     │
│  │ direct[3] = 15   │──────► block 15: data                    │
│  │ ...              │                                           │
│  └──────────────────┘                                           │
│                                                                  │
│  ✓ Random access is O(1) — go directly to any block            │
│  ✓ No external fragmentation                                    │
│  ✗ Index block overhead (mitigated by direct pointers)          │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Extent: (logical_start, physical_start, length)                │
│                                                                  │
│  File metadata:                                                 │
│  ┌──────────────────────────────────────────────────────┐      │
│  │ Extent 0: logical 0-99, physical 1000-1099 (100 blks)│      │
│  │ Extent 1: logical 100-149, physical 2000-2049 (50)   │      │
│  │ Extent 2: logical 150-200, physical 3000-3050 (51)   │      │
│  └──────────────────────────────────────────────────────┘      │
│                                                                  │
│  vs block pointers:                                             │
│  Block pointers for 100 blocks = 100 × 4 bytes = 400 bytes     │
│  One extent for 100 blocks = 12 bytes                          │
│                                                                  │
│  ✓ Efficient for large contiguous allocations                  │
...
│  │  │    .open, .read, .write, .llseek, .mmap, ...     │  │   │
│  │  ├───────────────────────────────────────────────────┤  │   │
│  │  │  struct inode_operations:                         │  │   │
│  │  │    .create, .lookup, .link, .unlink, .mkdir, ... │  │   │
│  │  ├───────────────────────────────────────────────────┤  │   │
│  │  │  struct super_operations:                         │  │   │
│  │  │    .statfs, .sync_fs, .write_inode, ...          │  │   │
│  │  └───────────────────────────────────────────────────┘  │   │
│  └─────────────────────────────────────────────────────────┘   │
│                        │                                        │
│      ┌─────────────────┼─────────────────┐                     │
...
│  │                    Page Cache                          │     │
│  │  ┌─────────────────────────────────────────────────┐  │     │
│  │  │  File A pages: [0][1][2][3]                     │  │     │
│  │  │  File B pages: [0][1]                           │  │     │
│  │  │  ...                                            │  │     │
│  │  └─────────────────────────────────────────────────┘  │     │
│  │                                                        │     │
│  │  Read: Check cache first → Hit: return cached page    │     │
│  │                          → Miss: read from disk, cache │     │
│  │                                                        │     │
│  │  Write: Write to cache, mark dirty                    │     │
│  │         Background writeback or sync flushes          │     │
│  └───────────────────────────────────────────────────────┘     │
│                        │                                        │
│                        ▼                                        │
│                    Block Device                                 │
│                                                                  │
│  Cache statistics: /proc/meminfo (Cached, Dirty, Writeback)   │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
<AccordionGroup>
  ...
  <Accordion title="Q5: Design a distributed file system">
    ...
    **Key decisions**:
    ...
    1. **Chunk size**: 64MB (reduces metadata, good for large files)
    
    2. **Replication**: 3 copies default, rack-aware placement
