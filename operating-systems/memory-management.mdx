...
## What is Memory Management? (From Scratch)

Imagine your computer's RAM (Random Access Memory) as a **warehouse** where programs store their data and code. Memory management is like having a **warehouse manager** who:

1. **Tracks what's stored where** - Knows which program uses which memory
2. **Allocates space** - Gives programs memory when they need it
3. **Prevents conflicts** - Ensures programs don't overwrite each other's data
4. **Reclaims space** - Frees memory when programs are done
5. **Optimizes usage** - Makes efficient use of limited physical memory

**Why is this important?**
- **Limited resource**: RAM is finite (typically 8-64 GB)
- **Many programs**: Dozens or hundreds of processes need memory
- **Performance**: How memory is managed affects speed dramatically
- **Security**: Memory isolation prevents programs from accessing each other's data

### The Memory Management Problem

**Without memory management:**
- Programs would need to know exact physical memory addresses
- Programs could overwrite each other's memory
- No way to run more programs than fit in RAM
- One program crash could corrupt others' data

**With memory management:**
- Programs use **virtual addresses** (don't care about physical location)
- OS maps virtual → physical addresses
- Programs are isolated (can't access each other's memory)
- OS can **swap** memory to disk when RAM is full
...
| Component | Latency (Time) | Latency (Scaled to Human Scale) |
|-----------|----------------|---------------------------------|
...
1.  **Splitting**: If a request is size $S$, find the smallest chunk size $2^k \ge S$. If the chunk is too big, split it in two equal buddies. Repeat until specific size matches.
...
**Merge Phase**:
If Process A frees its 128KB:
1.  Check buddy (next 128KB) -> It's FREE. **Merge** -> 256KB.
2.  Check buddy (next 256KB) -> It's FREE. **Merge** -> 512KB.
3.  Check buddy (next 512KB) -> It's FREE. **Merge** -> 1MB.
Result: Back to original 1MB contiguous block.
