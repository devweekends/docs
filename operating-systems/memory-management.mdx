...
│         │   ┌──────▼──────┐       │             │               │
│         ▼   │  SSD/Disk   │       ▼             ▼               │
│ Faster      │ (μs to ms)  │    Larger      Cheaper              │
│             └─────────────┘                                      │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
┌────────────────────────────────────────────────────────────────┐
│                                                                 │
│   CPU generates                    Physical memory              │
│   logical address                  is accessed                  │
│                                                                 │
│   ┌─────────┐    ┌───────────┐    ┌─────────────────────────┐  │
│   │  CPU    ├───►│   MMU     ├───►│   Physical Memory       │  │
│   │         │    │           │    │                         │  │
│   │ Logical │    │ Translates│    │   Physical Address      │  │
│   │ Address │    │ to physical│   │                         │  │
│   └─────────┘    └───────────┘    └─────────────────────────┘  │
│                                                                 │
│   Simple MMU: Physical = Logical + Base Register               │
│   Modern MMU: Uses page tables                                  │
│                                                                 │
└────────────────────────────────────────────────────────────────┘
```
...
│   │    OS (Reserved)     │  64 KB                           │   │
│   ├──────────────────────┼──────────────────────────────────┤   │
│   │    Partition 1       │  128 KB  ◄─── Process A (100KB)  │   │
│   │    (Internal frag)   │          28KB wasted             │   │
│   ├──────────────────────┼──────────────────────────────────┤   │
│   │    Partition 2       │  256 KB  ◄─── Process B (256KB)  │   │
│   │    (Fully used)      │          0KB wasted              │   │
│   ├──────────────────────┼──────────────────────────────────┤   │
│   │    Partition 3       │  512 KB  ◄─── Empty              │   │
│   │    (Unused)          │                                  │   │
│   └──────────────────────┴──────────────────────────────────┘   │
│                                                                  │
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│   Physical Memory (1 MB total)                                   │
│   ┌─────────────────────────────────────────────────────────┐   │
│   │    OS (Reserved)     │  64 KB                           │   │
│   ├──────────────────────┤                                  │   │
│   │    Process A         │  100 KB                          │   │
│   ├──────────────────────┤                                  │   │
│   │    Hole (Free)       │  50 KB   ◄─── External frag     │   │
│   ├──────────────────────┤                                  │   │
│   │    Process B         │  256 KB                          │   │
│   ├──────────────────────┤                                  │   │
│   │    Hole (Free)       │  30 KB   ◄─── External frag     │   │
│   ├──────────────────────┤                                  │   │
│   │    Process C         │  200 KB                          │   │
│   ├──────────────────────┤                                  │   │
│   │    Hole (Free)       │  300 KB  ◄─── Largest hole      │   │
│   └──────────────────────┴──────────────────────────────────┘   │
│                                                                  │
│   Total free: 380 KB but largest contiguous: 300 KB             │
│   Can't allocate 350 KB process!                                │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
```
Total free: 300 KB
But fragmented into: 50KB + 100KB + 150KB
Cannot allocate: 200 KB contiguous
...
│   Start: 1024 KB block                                          │
│                                                                  │
│   Request 100KB → allocate 128KB (next power of 2)             │
│                                                                  │
│   ┌─────────────────────────────────────────────────────────┐   │
│   │                        1024 KB                          │   │
│   └─────────────────────────────────────────────────────────┘   │
...
│         ▼ Split                                                  │
│   ┌─────┬──────┬─────────────┬─────────────────────────────┐    │
│   │128KB│128KB │   256 KB    │          512 KB             │    │
│   │ALLOC│ free │    free     │           free              │    │
│   └─────┴──────┴─────────────┴─────────────────────────────┘    │
│                                                                  │
│   On free: Merge with buddy if buddy is also free               │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
│   • Cache coloring for CPU cache efficiency                     │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
│                     ▼     │ ┌──┴───────────────────────┐    │   │
│   ┌───────────────┐ │     │ │                          │    │   │
│   │ Base = 1000   ├─┼────►│ │  Valid range for this   │    │   │
│   └───────────────┘ │     │ │  process: 1000-2000     │    │   │
│                     │     │ │                          │    │   │
│   ┌───────────────┐ │     │ │  Address 500 → 1500 ✓   │    │   │
│   │ Limit = 1000  ├─┘     │ │                          │    │   │
...
│   │ Limit (20 bits)                                         │   │
│   │ Type: Code/Data, Read/Write/Execute                     │   │
│   │ DPL: Descriptor Privilege Level (0-3)                   │   │
│   │ Present bit, Granularity, etc.                          │   │
│   └─────────────────────────────────────────────────────────┘   │
│                                                                  │
│   Protection checks:                                             │
│   • Segment limit check                                         │
│   • Type check (can't execute data segment)                     │
│   • Privilege level check (CPL ≤ DPL)                          │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
│   RAM                          Swap Space (Disk)                 │
│   ┌─────────────────┐          ┌─────────────────────────────┐  │
│   │    Process A    │ ◄──────► │   Process A (swapped out)  │  │
│   │    (Running)    │   swap   │                             │  │
│   ├─────────────────┤          ├─────────────────────────────┤  │
│   │    Process B    │          │   Process C (swapped out)  │  │
│   │    (Running)    │          │                             │  │
│   ├─────────────────┤          └─────────────────────────────┘  │
│   │    Free         │                                            │
│   │                 │                                            │
...
│   • Process blocked for long time                               │
│   • Priority-based decisions                                     │
│                                                                  │
│   Cost: Very slow (ms vs ns)                                    │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Purpose: Control device access to physical memory              │
│  • Similar to MMU for CPUs, but for I/O devices                 │
│  • Prevents rogue devices from accessing arbitrary memory       │
│  • Enables device virtualization (pass-through to VMs)          │
│                                                                  │
...
│  Without IOMMU:                                                 │
│  ┌─────────┐                                                    │
│  │ Device  ├───DMA───────►  Physical Memory (any address!)      │
│  └─────────┘               ⚠️  Security risk                    │
│                                                                  │
│  With IOMMU:                                                    │
│  ┌─────────┐    ┌────────┐    ┌────────────────────────┐       │
│  │ Device  ├───►│ IOMMU  ├───►│  Allowed Physical RAM  │       │
│  │         │    │        │    │  (per-device mapping)  │       │
│  └─────────┘    └────────┘    └────────────────────────┘       │
│                     │                                           │
│                     └──► Translation failed → Block access      │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
│                                                                  │
│  ┌─────────┐                                                    │
│  │  NIC    │ Wants to DMA to address 0x1000                     │
│  └────┬────┘                                                    │
│       │                                                         │
│       ▼ Device generates IOVA                                   │
│  ┌─────────────────────────────────────────┐                    │
│  │ IOVA: 0x1000 (device virtual address)  │                    │
│  └────────────────┬────────────────────────┘                    │
│                   │                                             │
│                   ▼                                             │
│  ┌──────────────────────────────────────────────┐               │
│  │           IOMMU Translation                  │               │
│  │  ┌────────────────────────────────────────┐  │               │
│  │  │ Device ID: 00:1f.6 (PCI address)      │  │               │
│  │  └─────────────┬──────────────────────────┘  │               │
│  │                ▼                              │               │
│  │  ┌──────────────────────────────┐            │               │
│  │  │  Context Entry (per-device)  │            │               │
│  │  │  • Page table pointer        │            │               │
│  │  │  • Domain ID                 │            │               │
│  │  │  • Translation type          │            │               │
│  │  └──────────────┬───────────────┘            │               │
...
│  │  ┌──────────────────────────────┐            │               │
│  │  │   IOMMU Page Tables          │            │               │
│  │  │   (similar to CPU paging)    │            │               │
│  │  │   IOVA → Physical Address    │            │               │
│  │  └──────────────┬───────────────┘            │               │
│  └─────────────────┼──────────────────────────┘│               │
│                    ▼                             │               │
│  ┌─────────────────────────────────────────────┐                │
│  │ Physical Address: 0x8CAFE000                │                │
│  └────────────────┬────────────────────────────┘                │
│                   │                                             │
│                   ▼                                             │
│  ┌──────────────────────────────────────────────┐               │
...
│  • Prevent device from accessing kernel memory                  │
│  • DMA isolation between VMs                                    │
│  • Interrupt remapping (prevent MSI attacks)                    │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────────┐
│              DMA REMAPPING SCENARIOS                             │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  1. DMA PROTECTION                                              │
│  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━                             │
│     Problem: Buggy/malicious device driver                      │
│     • Driver programs device with wrong address                 │
│     • Device DMAs to kernel memory → corruption                 │
│                                                                  │
│     Solution: IOMMU blocks unauthorized access                  │
│     • Device assigned limited address range                     │
│     • DMA outside range → IOMMU fault                           │
│     • System remains stable                                     │
│                                                                  │
│  2. DEVICE PASS-THROUGH TO VMs                                  │
│  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━                             │
│                                                                  │
│     ┌─────────────┐  ┌─────────────┐                            │
│     │   VM 1      │  │   VM 2      │                            │
...
│                     ▼                                            │
│     ┌────────────────────────────────┐                           │
│     │  Physical NIC (Assigned to VM) │                           │
│     └────────────────────────────────┘                           │
│                                                                  │
│     Benefits:                                                   │
│     • Near-native performance (no hypervisor overhead)          │
│     • Guest OS directly controls device                         │
│     • IOMMU ensures isolation between VMs                       │
│                                                                  │
│  3. BOUNCE BUFFERS ELIMINATION                                  │
│  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━                             │
│     Old 32-bit devices: Can only DMA to low 4GB                 │
│                                                                  │
│     Without IOMMU:                                              │
│     • Allocate bounce buffer in low memory                      │
│     • Copy data to/from bounce buffer                           │
│     • Performance hit                                            │
│                                                                  │
│     With IOMMU:                                                 │
│     • Device uses 32-bit addresses (limited)                    │
│     • IOMMU maps to any physical memory (64-bit)                │
│     • No copying needed!                                        │
│                                                                  │
│  4. SCATTER-GATHER DMA                                          │
│  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━                             │
│     Application buffer: Physically fragmented pages             │
│                                                                  │
│     Physical Memory:                                            │
│     ┌──────┐  ┌──────┐  ┌──────┐                                │
│     │Page 1│  │Page 2│  │Page 3│  (non-contiguous)              │
│     └───┬──┘  └───┬──┘  └───┬──┘                                │
│         │         │         │                                    │
│         └────┬────┴────┬────┘                                    │
│              ▼         ▼                                         │
│     ┌─────────────────────────────┐                              │
│     │  IOMMU creates contiguous   │                              │
│     │  IOVA mapping for device    │                              │
│     └──────────────┬──────────────┘                              │
│                    ▼                                             │
│     Device sees contiguous buffer!                              │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
│                                                                  │
│  IOTLB (I/O TLB):                                               │
│  • Caches IOVA → Physical translations                          │
│  • Similar to CPU TLB                                           │
│  • Miss = page table walk (expensive!)                          │
│                                                                  │
│  Typical overhead:                                              │
│  • IOTLB hit:   ~5-10 ns (negligible)                          │
│  • IOTLB miss:  ~100-500 ns (noticeable for high IOPS)        │
│                                                                  │
│  Optimization strategies:                                       │
│  1. Use huge pages for DMA buffers                             │
│     • Fewer IOTLB entries needed                               │
│     • Better hit rate                                           │
│                                                                  │
│  2. Pin memory for DMA                                          │
│     • Keep mappings stable                                      │
│     • Avoid IOTLB flushes                                       │
│                                                                  │
│  3. Batch IOMMU operations                                      │
│     • Map/unmap multiple pages at once                          │
│     • Reduce TLB shootdown overhead                             │
│                                                                  │
│  4. Use identity mapping for trusted devices                    │
│     • IOVA = Physical address                                   │
...
│                                                                  │
│  Disable IOMMU (if needed for performance):                     │
│  Boot parameter: intel_iommu=off (Intel)                        │
│                  amd_iommu=off  (AMD)                           │
│  ⚠️  Reduces security and virtualization features               │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
