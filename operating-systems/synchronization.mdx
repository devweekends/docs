...
|--------|----------|-----------|
| **Reader preference** | New readers can join if readers active | Writers may starve |
| **Writer preference** | Writers go ahead of waiting readers | Readers may starve |
| **Fair** | FIFO ordering | Lower throughput |
...
│    --- PREEMPTED ---                                            │
│                                                                  │
│  Thread 2: pop() A, pop() B, push() A (recycles A's memory)    │
│  Stack is now: A → C                                            │
│                                                                  │
│  Thread 1 resumes:                                              │
│    CAS(&top, A, B)  → SUCCEEDS! (top == A)                     │
│    Stack becomes: B → ???   (B was freed!)                     │
│                                                                  │
│  CORRUPTED!                                                     │
│                                                                  │
│  Solutions:                                                     │
│  1. Tagged pointers (add counter to pointer)                   │
│  2. Hazard pointers                                             │
