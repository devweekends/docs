...
description: "Curated recommendations for deep technical learning -- YouTube channels, system design books, database engineering, and weekly reading across the full stack"
icon: "tv"
---

The difference between engineers who plateau and engineers who keep growing is a consistent learning habit outside of work. This page collects the best resources across video, books, courses, and articles -- organized by topic so you can build a structured self-study plan. Each resource is included because it teaches the "why" behind engineering decisions, not just the "how."

# Channels

These YouTube channels are recommended because they explain engineering concepts with production context -- not just theory, but why decisions matter at scale.

- **Hussein Nasser** -- Databases and backend systems (YouTube). Known for deep dives into database internals, networking protocols, and backend architecture. His style is conversational and he frequently uses real-world scenarios (e.g., "what happens when Uber processes a ride request") to explain technical concepts. Start with his playlists on database indexing and connection pooling.
- **Jordan has no life** -- Engineering and dev workflow (YouTube). Covers system design concepts with a focus on practical implementation details and the trade-offs behind common architecture patterns. Good for engineers who want to go beyond surface-level system design explanations.

# System Design Books

These three resources cover system design from different angles. Use them in combination for the most complete preparation.

- **Designing Data-Intensive Applications** -- Martin Kleppmann. The definitive technical reference for understanding distributed systems, data storage, and stream processing. Dense but worth the investment. Read this for deep understanding of the principles; it is the book that senior engineers reference most often in design discussions. Not a quick read -- plan 2-3 months to work through it properly.
- **Acing the System Design Interview** -- Alex Xu. More directly targeted at interviews than DDIA. Walks through specific system design problems (design a URL shortener, design a chat system, design a news feed) with clear frameworks. Read this for interview-specific preparation and pattern recognition.
- **Educative.io System Design Courses** -- https://www.educative.io -- Interactive, browser-based courses with diagrams and quizzes. The "Grokking the System Design Interview" course is particularly popular for its structured approach to common interview problems. Useful as a complement to books when you want guided, step-by-step walkthroughs.

# Database Engineering

- **Fundamentals of Database Engineering** -- Hussein Nasser (Udemy): https://www.udemy.com/course/database-engines-crash-course/
  
  A comprehensive course covering database internals from fundamentals to advanced concepts. This is not a "how to write SQL" course -- it teaches you how databases actually work under the hood, which is the knowledge that separates database users from database engineers. Topics include:
  - ACID properties, transactions, and isolation levels (with practical demonstrations of what goes wrong when you choose the wrong isolation level)
  - Database indexing (B-Tree, B+Tree, LSM Trees, Bloom Filters) -- understanding these is critical for diagnosing slow queries in production
  - Partitioning strategies (horizontal, vertical, sharding) and when each approach makes sense
  - Replication patterns (master-slave, multi-master, synchronous vs async) and their implications for consistency and availability
  - Database engines internals (storage, caching, query execution) -- the "how does SELECT actually work" knowledge
  - PostgreSQL-specific deep dives and performance tuning
  - Concurrency control and locking mechanisms -- essential for building systems that handle concurrent users correctly
  - Database cursors, connection pooling, and query optimization

<Tip>
Hussein Nasser's YouTube channel complements this course with free content on database internals, backend architecture, and real-world case studies. Watch the free content first to see if his teaching style works for you before purchasing the course.
</Tip>

<Info>
Pair watch/read sessions with short notes and diagrams you can revisit before interviews. A 10-minute summary you write yourself is worth more than re-watching a 45-minute video when you need to refresh your memory.
</Info>

# Weekly Engineering Articles -- Curated Learning Resources

These articles are hand-picked to cover the topics that come up most frequently in technical interviews and day-to-day engineering work. Each summary explains not just what the article covers, but why it matters for your growth as an engineer. Read 2-3 per week to build a steady knowledge base.
...
Understanding JavaScript at the engine level separates developers who write code from developers who debug and optimize it. These resources cover the concepts that explain *why* JavaScript behaves the way it does.
...
System design is the most common topic in senior-level interviews and the skill that determines whether you can lead technical projects. These resources cover the fundamental patterns and trade-offs that every system design discussion is built on.
...
Backend engineering is where scalability, reliability, and performance decisions are made. These resources cover project structure, real-time communication patterns, and the operational concerns that production systems demand.
...
Understanding environment management is fundamental to shipping software reliably. These resources explain how professional teams structure their deployment pipelines and why each environment tier exists.
...
APIs are the contracts between systems. Understanding how to design, secure, and consume them is a core competency for any engineer working on web services or microservices.
...
Architecture decisions are the most expensive decisions in software -- they are hard to reverse and their consequences compound over time. These resources teach the principles that guide good architectural thinking.
...
Dependency injection is one of those design patterns that seems abstract until you experience the pain of testing tightly coupled code. Once you understand DI, your code becomes dramatically easier to test, modify, and maintain.
...
Concurrency bugs are among the hardest to diagnose in production because they are non-deterministic -- they only appear under specific timing conditions that are difficult to reproduce. These resources cover the theory and practical patterns for building systems that handle concurrent access correctly.
...
These resources cover the patterns and optimization techniques that separate junior React developers from engineers who can build performant, production-grade applications.
...
## JavaScript Performance and Multi-Threading

JavaScript's single-threaded nature is both its greatest strength (simplicity) and its biggest limitation (CPU-bound work blocks everything). These resources cover how to break through that limitation using Web Workers and worker threads -- essential knowledge for building responsive applications.
...
Database design decisions are notoriously expensive to change after your application is in production with real data. These resources cover the foundational concepts that prevent costly schema redesigns and the tools that make database interaction more productive.
...
AI engineering is the fastest-growing specialization in software development. These resources cover the practical skills -- API integration, prompt engineering, and understanding generative AI foundations -- that are now expected knowledge for many engineering roles, not just ML-specific ones.
...
Technical skills get you into interviews, but career skills determine how quickly you advance. These resources cover the professional positioning work that most engineers neglect -- and that creates outsized returns for those who invest in it.
