...
<Accordion title="What is the difference between sets and frozensets?">
  or `set()`. **Frozensets** are immutable versions created with `frozenset()`.
...
<Accordion title="How do you create a class in Python?">
  Use the `class` keyword followed by class name (PascalCase). Define `__init__`
  method as constructor. Use `self` parameter to refer to instance. Example:
  ```python class Person: def __init__(self, name, age): self.name = name
  self.age = age def greet(self): return f'Hello, I am {self.name}' p =
  Person('John', 30) ```
</Accordion>
...
<Accordion title="What are decorators in Python?">
  ...
  Example: ```python def my_decorator(func): def wrapper(*args, **kwargs):
  print('Before') result = func(*args, **kwargs) print('After') return result
  return wrapper ```
</Accordion>
...
<Accordion title="What is a closure?">
  ...
  ```python def outer(x): def inner(y): return x + y return inner add_5 =
  outer(5) print(add_5(10)) # 15 ```
</Accordion>
...
<Accordion title="Explain function annotations and their use cases">
  ...
  documentation, type checking tools (mypy), IDEs. Example: ```python def
  greet(name: str, age: int) -> str: return f"{name} is {age} years old" ``` Use
  cases: type hints, documentation, static analysis, IDE autocomplete.
</Accordion>
...
<Accordion title="How do you work with CSV files?">
  ...
  ```python import csv with open('data.csv', 'r') as f: reader = csv.reader(f)
...
<Accordion title="How do you create custom exceptions?">
  ...
  attributes and methods. Raise with `raise` keyword. Example: ```python class
  InvalidAgeError(Exception): def __init__(self, age, message='Age must be >=
  0'): self.age = age self.message = message super().__init__(self.message) ```
</Accordion>
...
<Accordion title="What are coroutines and async/await?">
  ...
  <Accordion title="What is the difference between comprehensions (list/dict/set/generator)?">
    Comprehensions provide concise syntax for creating collections. **List comprehension**: `[x**2 for x in range(10)]` returns list. **Dict comprehension**: `{k: v**2 for k, v in dict.items()}` returns dictionary. **Set comprehension**: `{x**2 for x in range(10)}` returns set. **Generator comprehension**: `(x**2 for x in range(10))` returns generator (lazy evaluation, memory efficient). All support conditional filtering with `if` clause. Generator comprehensions use parentheses and don't create full collection in memory.
  </Accordion>
...
<Accordion title="What are default arguments in functions?">
  Default arguments have preset values in function definition. Used when
  argument not provided in call. Must come after non-default arguments. WARNING:
  Don't use mutable defaults (lists, dicts) as they persist across calls.
...
<Accordion title="What is the difference between remove(), pop(), and del?">
  `remove(value)` removes first occurrence of value, raises ValueError if not
  found. `pop(index)` removes and returns item at index, default is last item.
  `del` statement removes item at index or deletes entire list. All modify list
  in place.
</Accordion>
...
<Accordion title="What is Python's memory model (pass-by-object-reference)?">
  ...
  operations create new objects. Example: ```python def modify_list(lst):
  lst.append(4) # modifies original def modify_int(x): x += 1 # creates new
  object, doesn't affect original my_list = [1, 2, 3] modify_list(my_list) #
  my_list becomes [1, 2, 3, 4] my_int = 5 modify_int(my_int) # my_int still 5
  ```
</Accordion>
...
<Accordion title="What is the difference between __getattr__ and __getattribute__?">
  **`__getattr__`**: called only when attribute not found through normal lookup
  ...
  Example: ```python class Example: def __getattr__(self, name): return
  f"Attribute {name} not found" def __getattribute__(self, name): # Called for
  ALL attribute access return object.__getattribute__(self, name) ``` Use
  `__getattr__` for fallback, `__getattribute__` for complete control (with
  caution).
</Accordion>
...
<Accordion title="How does generator execution and frame persistence work?">
  ...
  persists between calls. Example: ```python def counter(): i = 0 while True: i
  += 1 yield i # Pauses here, frame preserved gen = counter() print(next(gen)) #
  1 (frame created, executes to yield, pauses) print(next(gen)) # 2 (resumes
  from yield, frame still exists) ``` This frame persistence enables stateful
  generators and memory-efficient iteration over large datasets.
</Accordion>
...
<Accordion title="How does async/await differ from threading?">
  **Threading**: OS-managed, preemptive multitasking. Multiple threads, context
  ...
  switching overhead. More efficient for I/O-bound tasks. Example: ```python #
  Threading import threading def fetch(url): # blocking I/O return
  requests.get(url) # async/await async def fetch(url): # non-blocking I/O
  return await aiohttp.get(url) ``` async/await is more efficient for
...
<Accordion title="How do you profile and solve real-world performance bottlenecks?">
  ...
  premature optimization. Example: ```python import cProfile
  cProfile.run('your_function()') ``` Profile first, optimize based on data, not
  assumptions.
</Accordion>
...
<Accordion title="Why prefer composition over inheritance? Provide a practical refactoring example">
  ...
  test. Inheritance creates tight coupling. Example refactoring: ```python #
  Inheritance (bad) class Car(Engine, Wheels, Body): pass # Composition (better)
  class Car: def __init__(self): self.engine = Engine() self.wheels = Wheels()
  self.body = Body() ``` Composition allows changing components at runtime,
  easier testing (mock components), avoids diamond problem, more flexible.
</Accordion>
...
<Accordion title="What are four different ways to implement Singleton in Python, and their trade-offs?">
  **1) Module-level**: Python modules are singletons. Simplest. **2)
  Decorator**: function decorator that wraps class. Clean syntax. **3)
  Metaclass**: custom metaclass controls class creation. Powerful but complex.
  **4) `__new__` method**: override `__new__` in base class. Straightforward.
  Example (decorator): ```python def singleton(cls): instances = {}
  def get_instance(): if cls not in instances: instances[cls] = cls() return instances[cls]
  return get_instance ``` Trade-offs: module-level simplest but less flexible; decorator
  clean but adds indirection; metaclass powerful but complex; `__new__` straightforward
  but requires inheritance.
</Accordion>
...
<Accordion title="How do functions as first-class objects make Strategy pattern unnecessary?">
  ...
  ```python # Without Strategy pattern def process_data(data, strategy): return
  strategy(data) def sort_ascending(data): return sorted(data) def
  sort_descending(data): return sorted(data, reverse=True) # Usage result =
  process_data([3, 1, 2], sort_ascending) ``` Functions as first-class objects
  eliminate need for Strategy pattern classes in many cases, making code simpler
  and more Pythonic.
</Accordion>
...
<Accordion title="How do you write a context manager from scratch (class-based and @contextmanager)?">
  Two ways: **1) Class-based**: implement `__enter__` and `__exit__`. **2)
  Decorator**: use `@contextmanager`. Example: ```python # Class-based class
  DatabaseConnection: def __enter__(self): self.conn = connect() return
  self.conn def __exit__(self, exc_type, exc_val, exc_tb): self.conn.close() #
  @contextmanager from contextlib import contextmanager @contextmanager def
  database_connection(): conn = connect() try: yield conn finally: conn.close()
  ``` Both ensure proper resource cleanup. `@contextmanager` is simpler for
  basic cases.
</Accordion>
...
<Accordion title="What are key principles for designing RESTful APIs in Python?">
  **Resource-based URLs**: `/users/123` not `/getUser?id=123`. **HTTP methods**:
  GET (read), POST (create), PUT (update), DELETE (delete). **Stateless**: each
  request contains all information. **JSON responses**: standard format.
  **Versioning**: `/api/v1/users`. **Error handling**: proper HTTP status codes.
  **Pydantic models**: validate request/response. Use FastAPI or Flask-RESTful.
  Example: `@app.get('/users/{user_id}')` returns user, `@app.post('/users')`
  creates user.
</Accordion>
...
<Accordion title="How do you resolve circular dependencies?">
  Three main approaches: **1) Refactor**: move common code to third module. **2)
  Lazy imports**: import inside functions. **3) Dependency injection**: pass
  dependencies as parameters. Example: ```python # Bad: circular import # a.py
  imports b, b.py imports a # Solution 1: Refactor # common.py: shared code #
  a.py and b.py import common # Solution 2: Lazy import def func(): import b #
  Import when needed return b.something() # Solution 3: Dependency injection def
  func(dependency): return dependency.something() ``` Best approach: refactor to
  eliminate circular dependency through better architecture.
</Accordion>
...
<Accordion title="How do you debug Python code effectively?">
  Use Python debugger (`pdb`) or IDE debuggers. Set breakpoints, step through
  code, inspect variables. Use `print()` statements strategically (or logging).
  Use `traceback` module to analyze exceptions. Use `inspect` module to examine
  objects. Common techniques: **Breakpoints**: `import pdb; pdb.set_trace()`.
  **Logging**: use `logging` module instead of print. **Exception handling**:
  catch and log exceptions with context. **IDE debuggers**: PyCharm, VS Code,
  Jupyter notebooks. **Post-mortem debugging**: `python -m pdb script.py` after
  crash.
</Accordion>
...
<Accordion title="What is pdb and how do you use it?">
  ...
  ```python def calculate(x, y): import pdb; pdb.set_trace() # Breakpoint result
  = x + y return result ``` Essential for understanding code flow and finding
  bugs.
</Accordion>
...
<Accordion title="How do you use logging for debugging?">
  ...
  numbers, timestamps. Example: ```python import logging
  logging.basicConfig(level=logging.DEBUG, format='%(asctime)s - %(name)s -
  %(levelname)s - %(message)s') logger = logging.getLogger(__name__) def
  process_data(data): logger.debug(f"Processing {len(data)} items")
  logger.info("Starting processing") # ... code ... logger.error("Error
  occurred", exc_info=True) ``` Logging persists, can be filtered, and provides
  better debugging information than print statements.
</Accordion>
...
<Accordion title="How do you debug exceptions and tracebacks?">
  ...
  `sys.exc_info()` to get exception info. Example: ```python import traceback
  import sys try: # code that might fail result = 1 / 0 except Exception:
  traceback.print_exc() # Print to stderr exc_type, exc_value, exc_traceback =
  sys.exc_info() print(f"Exception type: {exc_type}") print(f"Exception value:{" "}
  {exc_value}") ``` Use `logging.exception()` to log exceptions with traceback.
  Understand stack traces: read from bottom up to find root cause.
</Accordion>
...
<Accordion title="What are common debugging techniques in Python?">
  ...
  **2) Debugger**: `pdb` or IDE debuggers. **3) Logging**: structured logging
  with context. **4) Exception handling**: catch and inspect exceptions. **5)
  Assertions**: verify assumptions with `assert`. **6) Profiling**: use
  `cProfile` to find bottlenecks. **7) Type checking**: use `mypy` to catch type
  errors. **8) Unit tests**: write tests to reproduce bugs. **9) Code review**:
  fresh eyes catch bugs. **10) Rubber duck debugging**: explain code to find
  issues. Combine techniques based on problem type.
</Accordion>

{" "}

<Accordion title="How do you debug performance issues in Python?">
  Use profiling tools: `cProfile` for function-level, `line_profiler` for
  line-level, `memory_profiler` for memory usage. Identify bottlenecks, then
  optimize. Example: ```python import cProfile import pstats profiler =
  cProfile.Profile() profiler.enable() # Your code here profiler.disable() stats
  = pstats.Stats(profiler) stats.sort_stats('cumulative') stats.print_stats(10)
  # Top 10 functions ``` Use `timeit` for micro-benchmarks. Use
  `memory_profiler` for memory leaks. Profile before optimizing: measure, don't
  guess. Focus on hot paths (code executed frequently).
</Accordion>
