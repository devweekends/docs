...
# The pipeline ensures scaling is fit ONLY on training data,
# preventing data leakage -- a subtle but critical detail.
pipeline = Pipeline([
...
<Warning>
**Always scale your data before using KNN!** Distance-based algorithms are sensitive to feature scales. Without scaling, a feature measured in thousands (like salary) will completely drown out a feature measured in single digits (like years of experience). The distance calculation will act as if only salary exists.

This applies to KNN, SVM, and any algorithm that uses distances. Tree-based models (Decision Trees, Random Forest, XGBoost) are immune to this problem because they split on one feature at a time.

See [Feature Engineering](/courses/ml-mastery/08-feature-engineering) for more on scaling strategies.
</Warning>
