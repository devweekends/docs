...
Navigation is one of the most critical aspects of mobile app development. Unlike the web where navigation is simply a URL change managed by the browser, mobile navigation involves native stack management, animated transitions, gesture handling (swipe back on iOS, hardware back button on Android), and memory management of screen instances. Getting this right determines whether your app feels like a polished native experience or a clunky web wrapper.
...
Combine multiple navigators for complex navigation patterns. This is where mobile navigation gets genuinely complex -- and where most bugs live. The key mental model: each navigator manages its own stack/state independently. When you nest a Stack inside a Tab, the Tab navigator does not know about the Stack's internal screens. This means the tab bar stays visible while you push screens within a tab's stack, which is usually exactly what you want.

A common pitfall: if you hide the Tab navigator's header but forget to show the Stack navigator's header (or vice versa), you end up with either double headers or no header at all.
...
Deep linking is what allows users to tap a link (in an email, text message, or another app) and land on a specific screen in your app rather than the home screen. On iOS, this works via URL schemes (`myapp://`) and Universal Links (`https://myapp.com/`). On Android, it uses Intent Filters and App Links. Expo Router handles most of this plumbing automatically.
...
Links like `myapp://user/123` will automatically route to `/user/123`. On iOS, make sure to test deep links by running `xcrun simctl openurl booted myapp://user/123` in the terminal. On Android, use `adb shell am start -a android.intent.action.VIEW -d "myapp://user/123"`.
...
This is essential for form screens, checkout flows, or any screen where leaving prematurely would lose user data. Note that on Android, the hardware back button and gesture navigation both trigger the `beforeRemove` event, while on iOS it is the swipe-back gesture and the header back button. Both platforms are covered by this single listener.
