...
print(f"\nRegression equation: Sales = {beta_0:.2f} + {beta_1:.2f} × AdSpend")

# Verify with scipy
slope, intercept, r_value, p_value, std_err = stats.linregress(ad_spend, sales)
print(f"Scipy: Sales = {intercept:.2f} + {slope:.2f} × AdSpend")
...
print(f"       + {model.coef_[1]:.2f} × Skills")
print(f"       + {model.coef_[2]:.2f} × Bay_Area")
...
<AccordionGroup>
  <Accordion title="Your regression model has R-squared of 0.45. A manager says that is terrible. How do you respond?">
    **Strong Answer:**
