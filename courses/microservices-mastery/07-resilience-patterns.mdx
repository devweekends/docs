...
- **Process-level concurrency cap.** Sum of all bulkhead sizes <= total concurrency the process can safely handle. Otherwise the bulkheads provide per-dependency isolation but not overall protection.
