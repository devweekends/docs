...
Where:
- $U$ = left singular vectors (patterns in **rows** — users/people)
- $\Sigma$ = singular values (how **important** each pattern is)
- $V^T$ = right singular vectors (patterns in **columns** — items/restaurants)
...
✅ **SVD** = decompose any matrix into U, Σ, V  
✅ **Low-rank approximation** = keep top k singular values  
✅ **Collaborative filtering** = predict missing ratings  
✅ **Hidden patterns** = discover latent factors  
✅ **Compression** = reduce storage with minimal loss  
