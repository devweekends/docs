...
**Edge Case 1: Injecting into non-NestJS classes**

If you have a plain TypeScript class (not registered as a provider) that needs a service, you cannot use constructor injection. Options: (1) Register the class as a provider; (2) Pass the dependency as a method parameter; (3) Use `ModuleRef` to resolve providers dynamically. Never use the `app.get()` hack to access providers outside the DI container -- it breaks testability and creates hidden dependencies.
...
Or use a factory provider that selects the implementation based on configuration.

**Edge Case 3: `onModuleInit` ordering between modules**
