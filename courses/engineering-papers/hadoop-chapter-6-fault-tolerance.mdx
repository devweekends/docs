...
The replication pipeline is one of Hadoop's most elegant design decisions, and it directly mirrors the chain replication approach described in the 2004 USENIX paper "Chain Replication for Supporting High Throughput and Availability" by van Renesse and Schneider. The key insight is that having the client send data to all replicas in parallel would saturate the client's outbound network link (it would need to send 3x the data). By chaining the writes -- client sends to node 1, node 1 forwards to node 2, node 2 forwards to node 3 -- the client only sends the data once, and the replication cost is distributed across the network. This same pattern appears in modern systems: Apache Kafka uses a leader-follower replication model (though followers pull rather than having the leader push), and CockroachDB's Raft-based replication chains writes through the leader to followers.
...
The heartbeat mechanism is the simplest form of failure detection in distributed systems, and it traces back to foundational work on group membership protocols. The trade-off is straightforward: frequent heartbeats detect failures faster but consume more network bandwidth and CPU. Hadoop's 3-second heartbeat interval with a 10-minute timeout represents a deliberate choice to avoid false positives (marking a healthy but temporarily slow node as dead) at the cost of slower failure detection. Compare this with Kubernetes, which uses a similar approach -- the kubelet sends heartbeats to the API server, and nodes are marked NotReady after a configurable timeout (default 40 seconds). The difference in timeout values reflects the different failure recovery costs: in Kubernetes, restarting a pod is fast and cheap, so aggressive timeouts are acceptable. In Hadoop, marking a DataNode dead triggers massive block re-replication, so conservative timeouts prevent unnecessary data movement.
...
The NameNode was Hadoop's most glaring single point of failure for years. Before Hadoop 2.0 (released in 2013), a NameNode crash meant the entire cluster was unusable until an operator manually restarted it and waited for it to replay the edit log and receive block reports from all DataNodes -- a process that could take 30 minutes or more on large clusters. This was Hadoop's equivalent of a "brain death" scenario. The Quorum Journal Manager (QJM) solution, contributed primarily by engineers at Cloudera, drew on the same quorum-based consensus principles that underpin Paxos and Raft. The epoch-based fencing mechanism is particularly important because it solves the "zombie leader" problem -- a challenge that every leader-election system must address. In modern cloud deployments, managed Hadoop services (Amazon EMR, Google Dataproc, Azure HDInsight) abstract this complexity away, but understanding the QJM architecture remains essential because the same patterns appear in every highly available stateful system, from etcd to CockroachDB to Apache BookKeeper.
