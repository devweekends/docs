...
HDFS is not a general-purpose file system -- and understanding what it deliberately does not do is as important as understanding what it does. Unlike POSIX-compliant file systems, HDFS sacrifices low-latency random access in favor of high-throughput sequential reads and writes. It does not support file modification in place (append-only semantics). It uses large block sizes (128MB by default, compared to 4KB in ext4) because it was designed for workloads that process multi-gigabyte files, not millions of small ones. These are not limitations born from laziness -- they are deliberate design choices inherited from GFS that optimize for the batch analytics workloads that Hadoop was built to serve. Yahoo's production HDFS clusters in 2008-2010 stored tens of petabytes across thousands of nodes, demonstrating that these trade-offs scaled to levels that traditional networked file systems like NFS or Lustre could not match economically.
...
<AccordionGroup>
  <Accordion title="Compare the HDFS NameNode with the GFS Master. Where did HDFS improve on the original design, and where did it fall short?">
    **Strong Answer:**

    HDFS improved on GFS in several ways. First, NameNode High Availability (added in HDFS 2.0): HDFS supports active/standby NameNode with automatic failover via ZooKeeper, reducing recovery time from minutes (GFS checkpoint replay) to seconds. This was critical because Hadoop served interactive workloads (Hive, HBase) that could not tolerate multi-minute metadata outages. Second, NameNode Federation: HDFS allows multiple NameNodes managing separate namespace volumes, partially addressing the single-master scalability limit. Third, HDFS has better operational tooling -- fsck for file system checks, balancer for even data distribution, and detailed JMX metrics -- because it was designed for a broader operator audience.

    Where HDFS fell short: it inherited the small files problem from GFS without any built-in mitigation. GFS had record append, which enabled multiple clients to write to a single large file concurrently, naturally producing large files. HDFS append support was minimal and unreliable for years, so applications tended to create many small files. Each file consumes about 150 bytes of NameNode heap, so millions of small files bloat the NameNode memory. Also, HDFS write semantics were simpler than GFS -- no equivalent of GFS pipelined chain replication optimization initially, and no atomic record append equivalent.

    **Follow-up: If the NameNode holds all metadata in memory, what happens when you reach the memory limit?**

    You hit a hard wall. The NameNode JVM cannot hold more metadata than the physical RAM allows. At 150 bytes per block with 128MB blocks, 1 billion blocks (128PB of data) requires about 150GB of NameNode heap. In practice, long GC pauses start causing problems well before you reach the theoretical limit -- typically around 60-80% heap utilization. Mitigation strategies: increase NameNode RAM (modern servers can have 1TB+), use NameNode federation to split the namespace, use HDFS erasure coding (which reduces the number of blocks by eliminating replication overhead), or archive cold data to object storage.
  </Accordion>

  <Accordion title="Walk me through exactly what happens during an HDFS write operation, step by step. Where can failures occur, and how does HDFS handle each one?">
    **Strong Answer:**

    Step 1: Client contacts the NameNode to create a file. NameNode checks permissions, creates the file entry in the namespace, and returns success. No blocks are allocated yet.

    Step 2: Client begins writing data. When the client buffer fills a block (128MB), it requests block allocation from the NameNode. The NameNode selects three DataNodes for replicas using rack-aware placement (first replica on the writer node or same rack, second on a different rack, third on the same rack as the second).

    Step 3: The client sets up a replication pipeline: Client connects to DN1, DN1 connects to DN2, DN2 connects to DN3. Data flows as a pipeline -- the client sends packets to DN1, which simultaneously writes to local disk and forwards to DN2, which writes and forwards to DN3.

    Step 4: Each DataNode sends an ACK back up the pipeline. DN3 ACKs to DN2, DN2 ACKs to DN1, DN1 ACKs to the client. The client only considers a packet committed when all three ACKs are received.

    Failure handling: If DN2 fails mid-write, the client detects the broken pipeline, removes DN2 from the pipeline, and tells the NameNode. The NameNode allocates a replacement DataNode. The client rebuilds the pipeline (Client -> DN1 -> DN3 -> DN_new) and resumes writing. Any data that was partially written to DN2 is garbage collected. The block version number is incremented so the stale partial block on DN2 (if it recovers) is detected and removed.

    If the NameNode fails during write, the client retries until the standby NameNode takes over (in HA mode) or until the NameNode recovers. Writes that were acknowledged before the failure are durable because the DataNodes have already stored the data.

    **Follow-up: What is the performance impact of the replication pipeline versus parallel writes to all three DataNodes?**

    The pipeline adds one network hop of latency (data goes Client -> DN1 -> DN2 -> DN3 sequentially) but uses each link at full bandwidth. Parallel writes would allow the client to send to all three simultaneously, but the client outbound bandwidth would be split three ways, resulting in slower aggregate transfer. For large sequential writes (the common HDFS case), the pipeline is more efficient because it saturates each network link. The total time is approximately (BlockSize / NetworkBandwidth) + 2 * RTT, compared to (3 * BlockSize / NetworkBandwidth) for parallel writes.
  </Accordion>

  <Accordion title="HDFS has a 'small files problem.' Explain why it exists, how it manifests in production, and what solutions are available.">
    **Strong Answer:**

    The small files problem exists because every file, directory, and block in HDFS is an object in the NameNode heap, consuming approximately 150 bytes. One 128GB file uses about 1,025 objects (1 file + 1,024 blocks), while 1 million 128KB files use 2 million objects -- 2,000x more memory for the same total data size. At scale, millions of small files can exhaust the NameNode heap.

    In production, this manifests in three ways. First, NameNode memory pressure: long GC pauses, out-of-memory crashes, or inability to create new files because the heap is full. Second, MapReduce overhead: each small file typically becomes one input split, launching a separate map task. Processing 1 million small files means 1 million JVM startups and task scheduling operations, which can take longer than processing the actual data. Third, NameNode startup time: loading millions of file entries from the fsimage during NameNode restart takes longer, increasing recovery time.

    Solutions: (1) Hadoop Archives (HAR files) -- bundle small files into a single archive with an index layer, reducing NameNode objects while preserving random access. (2) SequenceFiles -- use a container format where small files are stored as key-value pairs in a single large file. (3) CombineFileInputFormat -- a MapReduce input format that combines multiple small files into a single input split. (4) HBase -- store small records as rows in HBase, which manages its own storage as large HFiles on HDFS. (5) Compaction pipelines -- periodically merge small files into larger ones as a background job.

    **Follow-up: Why did HDFS not simply use a variable block size to handle small files better?**

    Because variable block sizes would complicate the NameNode metadata management significantly. Fixed block sizes allow uniform allocation, simple capacity planning, and predictable replication behavior. With variable blocks, the NameNode would need to track the size of every block, the balancer would need to account for different-sized blocks, and re-replication priority calculations would become more complex. The HDFS design philosophy is to keep the core simple and push complexity to higher-level tools and file formats.
  </Accordion>
</AccordionGroup>
