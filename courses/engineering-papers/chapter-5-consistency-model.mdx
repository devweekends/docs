...
GFS's consistency model is one of its most interesting and often misunderstood aspects -- and it is one of the most frequently discussed topics in system design interviews. Unlike traditional file systems that provide strong consistency (every read sees the latest write), GFS offers a relaxed model that trades some guarantees for higher performance and simpler implementation. This was a bold choice in 2003, when strong consistency was considered a non-negotiable requirement for any serious storage system. The GFS team's willingness to expose weaker guarantees to applications, and to co-design those applications to tolerate the weakness, was a watershed moment in distributed systems thinking. It directly paved the way for the "eventual consistency" models that dominate modern NoSQL databases (DynamoDB, Cassandra) and influenced the theoretical frameworks like PACELC that refined the CAP theorem. This chapter explores what GFS guarantees, what it doesn't, and how applications work with this model.
...
**Key Insight**: Consistency in GFS refers to **replica agreement**, not **data integrity relative to client intent**. This is why GFS strongly recommends "Record Append" for concurrent access, as it guarantees atomicity. This distinction between "replicas agree" and "the data is what I meant to write" is subtle but critically important, and it trips up even experienced engineers. In a system design interview, being able to articulate this distinction -- and explain why it matters practically -- signals deep understanding of distributed consistency.
...
<AccordionGroup>
  <Accordion title="Explain the difference between 'consistent' and 'defined' in GFS. Give me a concrete example of data that is consistent but undefined.">
    **Strong Answer:**

    In GFS, "consistent" means all replicas contain identical data -- if you read from any replica, you get the same bytes. "Defined" means consistent AND the data reflects what a specific client intended to write. Defined is a strict subset of consistent.

    Concrete example: Two clients simultaneously write to the same chunk at overlapping byte ranges. Client A writes "AAAAA" at offset 0, and Client B writes "BBBBB" at offset 0. If these writes are not atomic (they are regular writes, not record appends), the primary serializes individual network packets, and the replicas might all end up with "AABBA" -- the same interleaved data on every replica. This is consistent (all replicas agree) but undefined (neither client wrote "AABBA"). The data is garbage from the application perspective even though the replicas are perfectly in sync.

    This distinction is why GFS strongly recommends record append for concurrent writes. Record append guarantees that the entire record is written atomically at a GFS-chosen offset, so the result is always defined (the complete record appears intact) even when multiple clients append concurrently. The offset where each record lands may not be sequential, and there may be padding or duplicate records, but the data itself is never interleaved.

    **Follow-up: How does an application handle regions that are consistent but undefined?**

    Applications treat undefined regions as garbage and skip over them. The typical pattern is to embed record boundaries and checksums within the data itself. Each application-level record starts with a magic number, a length field, and a checksum. When reading, the application scans forward looking for valid magic numbers and verifies checksums. If a checksum fails, the record is skipped. This application-level framing adds overhead but makes the application resilient to both undefined regions and duplicate records from retried appends. MapReduce uses exactly this approach for its intermediate output files.
  </Accordion>

  <Accordion title="GFS record append may produce duplicate records and padding. How is that acceptable for a production system?">
    **Strong Answer:**

    Record append guarantees atomicity (the whole record is written or none of it is) but with two caveats: the record may appear more than once (due to retries after partial failures), and there may be padding bytes between records (to align to chunk boundaries). GFS considers this acceptable because the applications are designed to handle it.

    The application-level contract is: every record is self-describing, with a unique identifier, a checksum, and a length prefix. Consumers deduplicate by record ID and skip records with invalid checksums (which indicates padding or corrupted data). For MapReduce, this is trivial -- the framework already handles duplicate detection and idempotent processing.

    The alternative -- providing exactly-once append semantics -- would require distributed transactions or consensus for every append operation. For a system processing millions of appends per second from thousands of concurrent writers, the latency and complexity cost of exactly-once semantics would be prohibitive. GFS made the pragmatic choice to push deduplication to the application layer, where it can be done cheaply with a hash set of record IDs.

    This design pattern -- "at-least-once delivery with application-level idempotency" -- became the dominant approach in distributed systems. Kafka uses it. SQS uses it. Every modern event streaming system defaults to at-least-once delivery because exactly-once is either too expensive or requires end-to-end cooperation that crosses system boundaries.

    **Follow-up: In what scenario would duplicate records from GFS record append cause a correctness bug, even with deduplication?**

    If the application performs non-idempotent side effects based on the record content without checking for duplicates first. For example, if a financial system processes a "credit $100" record from GFS without deduplication, a retry could cause a double credit. The fix is straightforward -- assign unique transaction IDs and check a processed-set before applying -- but it requires the application developer to be aware of GFS semantics. This is the cost of co-design: the storage layer is simpler and faster, but application developers must understand and handle the relaxed guarantees.
  </Accordion>

  <Accordion title="How would you evolve GFS consistency model if you needed to support a real-time serving workload alongside batch MapReduce?">
    **Strong Answer:**

    The core tension is that batch workloads tolerate relaxed consistency for higher throughput, while real-time serving needs strong read-after-write consistency for correctness. I would introduce a tiered consistency approach.

    For the serving path, I would route reads through the primary replica (the one holding the lease), which guarantees seeing the most recent writes. This is essentially what a "strongly consistent read" means in DynamoDB. The cost is higher read latency (must contact the primary, not just any replica) and reduced read throughput (primary becomes a bottleneck for hot chunks).

    For the batch path, I would keep the existing relaxed model. MapReduce jobs can tolerate reading slightly stale data or handling duplicates.

    Architecturally, this is what Google actually did when they built Bigtable on top of GFS. Bigtable provides strong consistency for individual row operations by routing all reads and writes for a row through a single tablet server (similar to a primary replica). The underlying GFS still uses relaxed consistency for the SSTable files, but Bigtable layer enforces per-row consistency through its own locking and versioning.

    The broader lesson is that consistency is a spectrum, and different parts of your application stack can operate at different points on that spectrum. You do not need a single consistency model for the entire system.

    **Follow-up: Google eventually built Spanner for global strong consistency. What did GFS and Bigtable lack that motivated Spanner?**

    Cross-row transactions and global ordering. Bigtable provides strong consistency for single-row operations but not for multi-row transactions. Spanner uses TrueTime (GPS-synchronized atomic clocks) to provide globally consistent snapshots and cross-row transactions across data centers. The motivation was applications like AdWords and Google F1 (their ad-serving database) that needed ACID transactions across multiple tables. GFS consistency was too weak, and Bigtable consistency was too narrow (single-row only). Spanner paid for global consistency with higher write latency (cross-datacenter round trips), which was acceptable for the ad-serving workload but would be too slow for GFS-style batch processing.
  </Accordion>
</AccordionGroup>
