...
Data modeling in DynamoDB represents a fundamental paradigm shift from traditional relational database design, and it is the single area where most teams struggle when adopting DynamoDB. Rather than normalizing data and using joins (the approach every database course teaches), DynamoDB requires pre-joining data and denormalizing based on access patterns. This feels deeply wrong to anyone trained in relational modeling, and for good reason: in a relational database, normalization prevents data anomalies and gives you query flexibility. But DynamoDB makes a different trade-off: by requiring you to know your access patterns upfront and model for them explicitly, it guarantees predictable, single-digit-millisecond performance at any scale. You are trading query flexibility for performance predictability. This chapter explores the principles, patterns, and best practices for effective DynamoDB data modeling -- arguably the most important skill for any engineer working with DynamoDB in production.
...
Single-table design is a DynamoDB modeling technique popularized by Rick Houlihan (former AWS Principal Technologist) where you store multiple entity types in one table, using generic attribute names (PK, SK, GSI1PK, GSI1SK) and item type identifiers. This technique is controversial: some teams swear by it for its performance benefits, while others find it too complex to maintain. The key insight is that DynamoDB charges per request (not per table), so combining related entities into one table allows you to fetch them in a single Query operation, reducing both latency and cost. However, single-table design requires thorough upfront access pattern analysis and can be difficult to evolve when new access patterns emerge. In practice, many production teams use a hybrid approach: single-table design for entity groups that are frequently accessed together, and separate tables for unrelated domains.
