...
**Real-world analogy:** A spinlock is like standing at a bathroom door, repeatedly trying the handle every fraction of a second. You never sit down or go do something else -- you just keep jiggling the handle until it opens. This is fast when the bathroom is occupied for only a moment, but terrible if someone is in there for minutes. That is exactly why spinlocks must only protect operations that complete in nanoseconds.
 ...
 *
 * Key design insight: spinlocks trade CPU cycles for zero
 * context-switch overhead. A context switch costs ~1-10 microseconds;
 * a spinlock acquisition costs ~5-50 nanoseconds when uncontended.
 * This tradeoff only makes sense when the critical section is
 * shorter than a context switch.
 */
...
SpinLockAcquire(volatile slock_t *lock)
{
    ...
    while (tas(lock))  /* tas = "test-and-set": atomically check and try to claim */
    {
        ...
        if (++spins > NUM_SPINLOCK_DELAYS)
        {
             ...
             * never be held this long. If we reach here,
             * something is likely wrong (e.g., another process
             * holding the spinlock got context-switched out).
             *
             * pg_usleep(1) sleeps for 1 microsecond, giving
             * the holder a chance to finish and release.
             */
            pg_usleep(1);
            spins = 0;
        }
        ...
        {
            /* 
             * CPU pause instruction (e.g., x86 PAUSE / ARM YIELD).
             * Two benefits:
             * 1. Reduces power consumption during spin
             * 2. On hyperthreaded CPUs, yields execution resources
             *    to the sibling thread (which might be the lock holder!)
             */
            SPIN_DELAY();
        }
    }
...
**Real-world analogy:** If spinlocks are like jiggling a door handle, LWLocks are like taking a number at a deli counter. When the lock is busy, you sit down and wait to be called -- you do not burn energy standing at the counter. The cost is slightly higher latency (you need to be woken up), but you are not wasting CPU while waiting. LWLocks also support a shared/exclusive model: multiple readers can "look at the menu" simultaneously, but only one writer can "restock the display case" at a time.
...
**Practical tip:** WALInsertLock is the single most common bottleneck in write-heavy PostgreSQL workloads. If your monitoring shows frequent `LWLock:WALInsert` waits, you have hit the ceiling of how fast PostgreSQL can serialize writes to the write-ahead log. This is the database equivalent of a highway merging from 8 lanes into 1 -- every transaction must eventually funnel through WAL insertion.
 ...
 * The number of locks (8) was chosen to balance contention reduction
 * against the overhead of managing multiple insertion points.
 */
...
**Real-world analogy:** Heavyweight locks are like the booking system at a co-working space. Different activities require different levels of exclusivity: multiple people can read quietly at the same time (AccessShareLock), but if someone needs to renovate the entire floor (AccessExclusiveLock), everyone else has to leave. The lock manager tracks who is doing what, detects when two people have incompatible reservations, and resolves deadlocks -- something the simpler spinlocks and LWLocks cannot do.

**Common pitfall:** The name "heavyweight" does not mean "slow." It means "full-featured." These locks support 8 distinct modes, deadlock detection, and transaction-duration holding. The overhead is higher than LWLocks, but that is the price of the richer conflict model that user-visible operations require.
...
**Real-world analogy:** Imagine four cars arriving simultaneously at a four-way stop. Each car waits for the car to its right to go first. Nobody moves -- that is a deadlock. PostgreSQL detects this by building a "wait-for graph" (who is waiting for whom) and checking for cycles. When it finds one, it picks a victim -- the equivalent of one car backing up so the others can proceed.

**Practical tip:** Deadlock detection is deliberately delayed by `deadlock_timeout` (default 1 second) because the detection algorithm itself is expensive. It must freeze the lock manager state and walk the entire wait-for graph. In most cases, the lock is granted before the timeout fires, so the expense is avoided entirely. If you see frequent deadlock detections in your logs, the fix is almost always in your application's lock ordering, not in tuning this parameter.
...
**The golden rule of deadlock prevention: always acquire locks in the same global order.** If every transaction acquires locks on `accounts` before `orders`, deadlocks between those two tables become structurally impossible. This is the same principle that operating systems use to prevent resource deadlocks -- define a total ordering and never violate it.
...
**Real-world analogy:** Advisory locks are like putting a "Do Not Disturb" sign on a hotel room door. The hotel does not enforce it -- the lock manager does not prevent access to any database object. But other well-behaved guests (application processes) will check for the sign before entering. Advisory locks give your application a cooperative locking mechanism built into the database, eliminating the need for an external tool like Redis or ZooKeeper for simple coordination tasks.

**When to use advisory locks over row-level locks:** Use advisory locks when you need to synchronize operations that do not map cleanly to a single row or table -- such as preventing duplicate cron job execution, rate limiting per user, or coordinating access to an external resource like a payment gateway.
...
| Feature | Database (PostgreSQL) | External DLM (Redis/etcd) |
| :--- | :--- | :--- |
| **Integrity** | ACID compliant; lock released on rollbacks. | Independent of DB state; risky if DB tx fails. |
| **Performance** | High overhead for massive scale. | Ultra-low latency (in-memory). |
| **Complexity** | Low (use `SELECT FOR UPDATE`). | High (requires separate infrastructure). |
| **Use Case** | Inventory, financial balances. | Global rate limiting, leader election. |
...
<AccordionGroup>
  <Accordion title="Explain the three levels of PostgreSQL's lock hierarchy. Why does the system need spinlocks when it already has LWLocks?">
    **Strong Answer:**

    - The three levels are spinlocks (CPU-level atomic operations, nanosecond hold times, busy-wait), LWLocks (shared/exclusive modes, microsecond to millisecond hold times, can sleep), and heavyweight locks (row-level and table-level locks visible to users, can be held for the duration of a transaction).
    - Spinlocks exist because LWLocks themselves need mutual exclusion to update their internal state (wait queue management, state transitions). You cannot use an LWLock to protect another LWLock's internals without creating a circular dependency. Spinlocks provide the lowest-level atomic primitive: a single compare-and-swap instruction that either succeeds or busy-waits. They are held for nanoseconds -- just long enough to update a flag or pointer.
    - The hierarchy is strict: code holding a spinlock must never acquire an LWLock (risk of sleeping while holding a spinlock, which would waste CPU and potentially cause priority inversion). Code holding an LWLock must follow a documented ordering to prevent deadlocks. Heavyweight locks have their own deadlock detection mechanism.

    **Follow-up: What happens when you see LWLock:WALInsert contention in pg_stat_activity?**

    WALInsert lock contention means multiple backends are competing to write WAL records simultaneously. This is common on high-throughput write workloads. PostgreSQL uses multiple WAL insertion locks (configured by `wal_insert_locks`, typically 8) to allow limited parallelism, but under extreme write pressure, backends queue. Solutions: reduce WAL volume (combine small transactions into batches, use UNLOGGED tables for non-critical data), enable wal_compression, or move to faster storage for the WAL directory (dedicated NVMe for pg_wal).
  </Accordion>

  <Accordion title="A production query is hanging indefinitely. pg_stat_activity shows it is waiting on a Lock event. Walk through your diagnosis.">
    **Strong Answer:**

    - Step 1: Identify what lock the query is waiting for. `SELECT pid, mode, relation::regclass, page, tuple FROM pg_locks WHERE NOT granted` shows all pending lock requests. The `relation` column tells you which table, `mode` tells you the lock type (RowExclusiveLock, AccessShareLock, etc.).
    - Step 2: Find who is holding the conflicting lock. `SELECT blocked.pid AS blocked_pid, blocking.pid AS blocking_pid, blocking.query AS blocking_query, blocking.state, blocking.query_start FROM pg_stat_activity blocked JOIN pg_locks bl ON blocked.pid = bl.pid JOIN pg_locks hl ON bl.relation = hl.relation AND bl.locktype = hl.locktype AND NOT bl.granted AND hl.granted JOIN pg_stat_activity blocking ON hl.pid = blocking.pid WHERE blocked.pid = <your_pid>`. This shows the blocking query, how long it has been running, and its current state.
    - Step 3: Common scenarios: (a) The blocker is `idle in transaction` -- someone opened a transaction and walked away. Kill it with `SELECT pg_terminate_backend(<blocking_pid>)`. (b) The blocker is a long-running DDL (ALTER TABLE, CREATE INDEX without CONCURRENTLY) holding an AccessExclusive lock. You must wait for it to finish or cancel it. (c) Deadlock -- but PostgreSQL should detect this within `deadlock_timeout` seconds automatically.
    - Step 4: Prevention. Set `idle_in_transaction_session_timeout = '5min'` to automatically terminate idle transactions. Set `lock_timeout = '30s'` per session so queries fail fast rather than waiting indefinitely. Use `statement_timeout` as a safety net.

    **Follow-up: What is the difference between advisory locks and regular locks, and when would you choose advisory locks?**

    Regular locks are automatically acquired and released by PostgreSQL as part of SQL operations (SELECT acquires AccessShareLock, UPDATE acquires RowExclusiveLock). Advisory locks are explicitly acquired and released by application code -- PostgreSQL does not care what they "mean." Use advisory locks for application-level coordination: preventing duplicate cron job execution (`SELECT pg_try_advisory_lock(hashtext('nightly-report'))`), serializing access to an external resource (payment processing), or implementing optimistic locking patterns. Advisory locks participate in deadlock detection but are not tied to any specific table or row.
  </Accordion>

  <Accordion title="How does PostgreSQL detect deadlocks, and what is the performance cost of deadlock detection?">
    **Strong Answer:**

    - PostgreSQL's deadlock detector runs as a background task triggered when any backend has been waiting for a lock longer than `deadlock_timeout` (default 1 second). It builds a wait-for graph by examining all lock waiters and lock holders in the shared lock table. If it finds a cycle in this graph (A waits for B, B waits for C, C waits for A), it selects a victim and terminates that transaction with `ERROR: deadlock detected`.
    - The victim selection is heuristic: PostgreSQL picks the transaction that would be "cheapest" to abort, considering factors like the number of locks held and work done. The terminated transaction's locks are released, allowing the others to proceed.
    - Performance cost: the deadlock detection itself is relatively cheap -- it is a graph traversal over the lock table, which is in shared memory. The cost is that it only runs after `deadlock_timeout`, meaning every potential deadlock causes at least 1 second of wasted waiting. Setting `deadlock_timeout` too low (e.g., 100ms) causes the detector to run frequently even for normal lock waits that would resolve on their own, wasting CPU. Setting it too high (e.g., 30s) means genuine deadlocks cause 30 seconds of blocked transactions before resolution.
    - The real cost of deadlocks is not detection but the wasted work: the victim transaction has its work rolled back, and it (or the application) must retry from scratch. In high-contention systems, frequent deadlocks indicate a design problem -- fix the lock ordering rather than tuning the detection interval.

    **Follow-up: Can deadlocks happen between LWLocks, not just heavyweight locks?**

    LWLock deadlocks are prevented by design rather than detected at runtime. The PostgreSQL source code defines a strict ordering for LWLock acquisition (documented in lwlock.h). Developers must always acquire LWLocks in the prescribed order. If code needs locks A and B, it must always acquire A before B. Violating this ordering is considered a bug in the PostgreSQL source code. This is verified during development but not enforced at runtime, which is why LWLock-related deadlocks, while theoretically possible due to bugs, are extremely rare in practice.
  </Accordion>
</AccordionGroup>
