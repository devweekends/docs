...
│   │  ┌─────────────────────────────────────────────────────────────┐   │   │
│   │  │ pd_lsn (8)      - LSN of last WAL record affecting page    │   │   │
...
│   │  │ pd_special (2)  - Offset to special space (index-specific) │   │   │
│   │  │ pd_prune_xid (4)- Oldest prunable XID                      │   │   │
│   │  │ pd_linp[0..N]   - Line pointer array (ItemId, 4 bytes each)│   │   │
│   │  └─────────────────────────────────────────────────────────────┘   │   │
│   │                                                                     │   │
│   │  Line Pointers (grow downward from header)                         │   │
│   │  ┌─────┬─────┬─────┬─────┬─────┐                                   │   │
│   │  │ LP1 │ LP2 │ LP3 │ LP4 │ ... │  ← pd_lower points here          │   │
│   │  └──┬──┴──┬──┴──┬──┴──┬──┴─────┘                                   │   │
│   │     │     │     │     │                                             │   │
│   │     │     │     │     │         FREE SPACE                         │   │
│   │     │     │     │     │                                             │   │
...
│   │     ▼                                 ▼   ▼   ▼ ▼                   │   │
│   │  ┌──────────┬──────────┬──────────┬──────────┐                     │   │
│   │  │  Tuple 4 │  Tuple 3 │  Tuple 2 │  Tuple 1 │  (grow upward)     │   │
│   │  └──────────┴──────────┴──────────┴──────────┘                     │   │
│   │                                                                     │   │
│   │  ┌─────────────────────────────────────────────────────────────┐   │   │
...
-- 0/1A5B7C8  |    12345 |     0 |    48 |  7920 |    8192 |     8192 |       4 |     1234

-- View line pointers (item identifiers)
SELECT * FROM heap_page_item_attrs(get_raw_page('users', 0), 'users');

-- View individual tuple data
...
│   │   t_cid (4 bytes)       - Command ID (insert or delete)            │   │
│   │                          OR t_xvac for VACUUM                       │   │
│   │   t_ctid (6 bytes)      - Current TID of this or newer tuple       │   │
│   │                          (block number + offset)                    │   │
...
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │   Bit per attribute: 1 = NOT NULL, 0 = NULL                        │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                                                                              │
│   OID (optional, 4 bytes) - only if table has OIDs (deprecated)            │
│                                                                              │
│   User Data (attribute values)                                              │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │   Attribute 1 value                                                 │   │
