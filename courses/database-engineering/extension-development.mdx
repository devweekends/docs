...
<AccordionGroup>
  <Accordion title="You need to add custom functionality to PostgreSQL. When would you write an extension versus a PL/pgSQL function versus an external service?">
    **Strong Answer:**

    - **PL/pgSQL function**: Use for business logic that operates on data already in the database and does not need low-level access. Examples: complex validation rules, data transformation pipelines, stored procedures. Advantages: no compilation needed, version-controlled with migration scripts, sandboxed within PostgreSQL's security model. Limitation: interpreted, not suitable for CPU-intensive computation.
    - **C extension**: Use when you need: (1) custom index types or access methods, (2) hooks into the planner or executor, (3) background workers that run independently of client connections, (4) performance-critical functions that PL/pgSQL cannot deliver (C is 10-100x faster for computation), or (5) integration with external C libraries (compression, ML inference). Examples: PostGIS (spatial indexes), pg_stat_statements (query tracking), pgvector (vector similarity search).
    - **External service**: Use when the functionality requires: (1) independent scaling (ML inference at GPU scale), (2) different runtime (Python, Java), (3) state that does not belong in the database (session management), or (4) the risk of a bug crashing PostgreSQL is unacceptable. Call via Foreign Data Wrappers, HTTP extensions (pg_net), or application-level integration.
    - Decision framework: if it manipulates data in-place and needs transactional consistency, keep it in PostgreSQL (function or extension). If it is compute-heavy and stateless, external service. If it needs to modify PostgreSQL's behavior (custom operators, index methods, auth hooks), it must be a C extension.

    **Follow-up: What are the risks of deploying a C extension in production?**

    A bug in a C extension runs in the same process as PostgreSQL. A segfault crashes the backend (or worse, the postmaster). A memory leak in TopMemoryContext accumulates across queries. A lock ordering violation can cause deadlocks. Mitigation: extensive testing, memory context discipline (never allocate in TopMemoryContext from query-time code), use PGDLLEXPORT correctly, test with AddressSanitizer during development, and deploy to a staging replica before production.
  </Accordion>

  <Accordion title="How do PostgreSQL hooks work, and what can you intercept with them?">
    **Strong Answer:**

    - PostgreSQL exposes function pointer hooks at key points in query processing. An extension can replace these pointers with its own functions, intercepting and optionally modifying behavior. The extension typically saves the original pointer and chains to it after adding its custom logic.
    - Key hooks: `planner_hook` (intercept query planning -- used by pg_hint_plan to inject optimizer hints), `ExecutorStart_hook` / `ExecutorEnd_hook` (wrap query execution -- used by pg_stat_statements to track timing), `ProcessUtility_hook` (intercept DDL commands), `emit_log_hook` (intercept log messages for custom routing), `auth_delay_hook` (add delays to failed authentication for brute-force protection).
    - How it works: PostgreSQL defines global function pointers (e.g., `planner_hook_type planner_hook = NULL`). When the hook is NULL, the default implementation runs. An extension's `_PG_init()` function saves the current pointer and installs its own: `prev_planner = planner_hook; planner_hook = my_planner;`. The custom function does its work and calls `prev_planner()` to chain to the previous handler.
    - The limitation: hooks are not a stable API. They can change between major PostgreSQL versions. Extensions relying on hooks must be recompiled and potentially modified for each major version upgrade. There is no ABI compatibility guarantee.

    **Follow-up: Can two extensions hook the same function pointer? What happens?**

    Yes, through chaining. If extension A installs its hook first, and extension B installs second, B's hook calls A's hook (which B saved as `prev`), and A's hook calls the original PostgreSQL function. The execution order is reverse-installation-order (B runs first, then A, then PostgreSQL default). The order depends on `shared_preload_libraries` ordering in postgresql.conf. This works well when hooks are additive (both pg_stat_statements and auto_explain can hook ExecutorEnd without conflict). It breaks when hooks are mutually exclusive (two extensions trying to replace the planner with incompatible logic).
  </Accordion>

  <Accordion title="Walk me through the lifecycle of a PostgreSQL extension from CREATE EXTENSION to DROP EXTENSION.">
    **Strong Answer:**

    - **CREATE EXTENSION myext**: PostgreSQL reads `myext.control` (name, version, schema, requires) from the extension directory. It locates `myext--1.0.sql` and executes it within a transaction. This SQL script creates functions, types, operators, and tables. If the extension has a shared library (`module_pathname`), it is loaded via `dlopen`/`LoadLibrary`, and the library's `_PG_init()` function is called (once per backend, on first load). All objects created are tagged as belonging to the extension in pg_depend.
    - **Using the extension**: Functions call into the shared library via the PostgreSQL function call interface (V1 calling convention, SPI for SQL execution from C). The shared library runs in the same address space as the backend. Memory allocated via palloc is managed by PostgreSQL's context system.
    - **ALTER EXTENSION myext UPDATE TO '1.2'**: PostgreSQL finds the shortest upgrade path from current version to 1.2 using available upgrade scripts (e.g., `myext--1.0--1.1.sql`, `myext--1.1--1.2.sql`). It executes them in sequence within a transaction. If any script fails, the entire upgrade rolls back.
    - **DROP EXTENSION myext**: PostgreSQL looks up all objects owned by the extension via pg_depend and drops them in dependency order. The shared library remains loaded in memory until the backend exits (there is no dlclose for PostgreSQL extensions). The control and SQL files remain on disk.
    - Key production consideration: CREATE EXTENSION and ALTER EXTENSION run DDL within a transaction and take appropriate locks on the objects being created or modified. For large extensions that create many objects, this can block concurrent operations briefly.

    **Follow-up: What happens if the shared library crashes? Does it take down the entire database?**

    A crash (segfault) in the shared library terminates the backend process that triggered it. The postmaster detects the child process death and restarts the backend infrastructure. Other backends continue operating normally -- this is the process isolation advantage of PostgreSQL's multi-process model. However, if the crash corrupts shared memory (e.g., writing to a random address in the shared buffer pool), the postmaster detects this via the crash recovery mechanism and restarts ALL backends. This is rare but possible, which is why untested extensions should never be deployed on production primaries without staging validation.
  </Accordion>
</AccordionGroup>
