...
<AccordionGroup>
  <Accordion title="You are designing a schema for a multi-tenant SaaS application. How do you handle tenant isolation at the database level, and what are the tradeoffs of each approach?">
    **Strong Answer:**

    - There are three primary strategies, each with distinct tradeoffs in isolation, operational complexity, and cost:
    - **Database-per-tenant**: Each tenant gets their own PostgreSQL database. Maximum isolation -- one tenant's runaway query cannot affect others. Backup, restore, and compliance (GDPR data deletion) are trivial per-tenant operations. The cost is operational: managing hundreds or thousands of databases means automated provisioning, monitoring multiplied by N, and connection pooling complexity. This works well for enterprise SaaS with fewer than ~1000 tenants.
    - **Schema-per-tenant**: All tenants share one database but each gets their own schema. Moderate isolation -- you get separate tables and indexes per tenant, and pg_dump can export a single schema. Cross-tenant queries are possible via schema-qualified names. The downside is that PostgreSQL catalog bloat becomes a real problem beyond ~5000 schemas, as the system catalog tables (pg_class, pg_attribute) grow enormous and plan cache performance degrades.
    - **Shared tables with tenant_id column**: All tenants share the same tables with a tenant_id discriminator. This is the most operationally simple and scales to millions of tenants, but requires careful attention to Row-Level Security (RLS) policies to prevent data leakage, and a single large table means vacuum and index maintenance affect everyone. Always include tenant_id as the leading column in every index and in the primary key.
    - My default recommendation: start with shared tables plus RLS for small/medium SaaS. Move large enterprise tenants to schema-per-tenant or database-per-tenant when their data volume or compliance requirements justify the overhead.

    **Follow-up: How would you prevent a noisy neighbor from degrading performance for other tenants in the shared-table model?**

    Three mechanisms: (1) Per-tenant connection limits using pg_hba.conf and connection pooling rules so one tenant cannot exhaust the connection pool. (2) Statement timeout set per tenant role so a runaway analytics query cannot hold resources. (3) Resource groups or the pg_cron extension for throttling vacuum and maintenance per large tenant. In extreme cases, use Citus to shard by tenant_id, which gives you physical data separation while maintaining the shared-table programming model.
  </Accordion>

  <Accordion title="When is denormalization the right answer, and how do you avoid the usual pitfalls?">
    **Strong Answer:**

    - Denormalization is the right answer when the read frequency vastly exceeds the write frequency for a particular access pattern, and the normalized form requires expensive multi-table JOINs that cannot be made fast enough with indexes alone. Classic examples: a user's display name stored on every comment row to avoid joining the users table on a feed page rendering 500 comments, or a pre-computed order_total stored on the orders table to avoid summing line_items on every dashboard load.
    - The pitfalls are well-known: update anomalies (you change a user's display name and must update 50,000 comment rows), storage bloat, and the insidious bug where the denormalized copy drifts from the source of truth due to a missed code path.
    - The mitigation strategies I use: (1) Triggers to keep denormalized columns in sync -- a trigger on users that updates all comments when display_name changes. This is reliable but adds write latency. (2) Materialized views for read-heavy aggregations that can tolerate bounded staleness. REFRESH MATERIALIZED VIEW CONCURRENTLY lets readers continue while the view rebuilds. (3) Generated columns (PostgreSQL 12+) for computed values derived from the same row. (4) Application-level eventually-consistent sync via CDC (Change Data Capture) for complex cross-table denormalization.
    - The key principle: every denormalized value should have a clearly documented "source of truth" and a defined mechanism for keeping it in sync. If you cannot point to both, the denormalization will eventually cause data corruption.

    **Follow-up: How do you decide between a materialized view and a denormalized column?**

    Materialized views are better for complex aggregations across many tables where the result is consumed as a read-only report or dashboard. They have clear refresh semantics and do not pollute the transactional schema. Denormalized columns are better for single-row lookups that must be fast and always consistent -- like showing an order's customer_name without a JOIN. The rule of thumb: if the denormalized data needs to be part of a transactional query (JOIN, WHERE filter), use a column. If it is a read-only reporting artifact, use a materialized view.
  </Accordion>

  <Accordion title="Walk me through how you would design a schema to handle polymorphic entities -- for example, a notification system where notifications can reference orders, comments, or user profiles.">
    **Strong Answer:**

    - There are four common patterns, and I have strong opinions about which to use:
    - **Separate foreign key columns**: `order_id INT NULL, comment_id INT NULL, profile_id INT NULL` with a CHECK constraint ensuring exactly one is non-null. Simple and type-safe with proper foreign keys, but the table gets wider with every new entity type. Works well when you have fewer than 5-6 entity types.
    - **Generic foreign key (entity_type + entity_id)**: `entity_type VARCHAR, entity_id BIGINT`. This is the Rails/Django default and it is the worst pattern for databases because you cannot create a foreign key constraint on a polymorphic reference. You lose referential integrity and must rely on application code to prevent orphaned records. I avoid this pattern unless the application framework provides bulletproof lifecycle management.
    - **Join table per entity type**: A notifications table with separate join tables like notification_orders, notification_comments, etc. Fully normalized with proper foreign keys. More tables but maximum type safety. This is my preference for data-critical systems.
    - **JSONB payload**: Store the reference as `{"type": "order", "id": 12345}` in a JSONB column. Maximum flexibility but zero database-level integrity. Use GIN index for lookups. This works well for event/activity streams where the referenced entity types change frequently and referential integrity is managed at the application level.

    **Follow-up: What about using PostgreSQL table inheritance for polymorphism?**

    PostgreSQL supports table inheritance (`CREATE TABLE vip_users () INHERITS (users)`) but I would not recommend it for polymorphism in production. It has significant limitations: foreign keys do not propagate to child tables, unique constraints are not enforced across the hierarchy, and the query planner has to scan all child tables for queries on the parent. Partitioning (which uses a similar syntax) is well-supported and performant, but general-purpose inheritance is a legacy feature that the PostgreSQL community itself discourages for new designs.
  </Accordion>
</AccordionGroup>
