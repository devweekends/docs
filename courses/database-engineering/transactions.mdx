...
─────────────────────               ─────────────────────
1. Deduct $100 from Alice ✓         BEGIN
2. Server crashes! 💥               1. Deduct $100 from Alice ✓
3. Bob never receives money         2. Server crashes! 💥
                                    3. ROLLBACK (automatic)
Result: Alice lost $100             Result: No change, retry safely
```

---

## 3.2 ACID Properties Deep Dive

ACID is the gold standard for transaction processing. Let's understand each property in depth.

### Atomicity: All or Nothing

<Tabs>
  <Tab title="Concept">
    **Atomicity** guarantees that all operations in a transaction either complete entirely or have no effect at all. There's no middle ground.
    ...
    │   ┌─────────────────────────────────────────────────────────┐   │
    │   │  Operation 1: UPDATE sender SET balance -= 100          │   │
    │   │  Operation 2: UPDATE receiver SET balance += 100        │   │
    │   │  Operation 3: INSERT INTO transfer_log (...)            │   │
    │   └─────────────────────────────────────────────────────────┘   │
    │                           │                                     │
    │              ┌────────────┴────────────┐                       │
    │              ▼                         ▼                       │
    ...
    │   │                  │     │                  │                │
    │   │  COMMIT          │     │  ROLLBACK        │                │
    │   │  All 3 changes   │     │  No changes      │                │
    │   │  are permanent   │     │  are applied     │                │
    │   └──────────────────┘     └──────────────────┘                │
    │                                                                 │
    └─────────────────────────────────────────────────────────────────┘
    ```
...
<Tabs>
  ...
  <Tab title="Types of Consistency">
    **Database-Level Consistency** (enforced by DBMS):
    ...
    **Application-Level Consistency** (your responsibility):
    - Business rules (order total = sum of items)
    - Cross-table invariants
    - Complex validation logic
    
    ```sql
    -- Application-level: Ensure order total matches items
    BEGIN;
    
    INSERT INTO orders (id, user_id, total) VALUES (1, 100, 0);
    
    INSERT INTO order_items (order_id, product_id, qty, price)
    VALUES (1, 10, 2, 50.00), (1, 20, 1, 25.00);
    
    -- Update total to match items
    UPDATE orders 
    SET total = (SELECT SUM(qty * price) FROM order_items WHERE order_id = 1)
    WHERE id = 1;
...
<Tabs>
  <Tab title="Concept">
    **Isolation** ensures concurrent transactions don't interfere with each other. Each transaction sees a consistent view of the data.
    ...
    │  Without isolation: might be $700 or $800 (lost update)         │
    │                                                                 │
    └─────────────────────────────────────────────────────────────────┘
    ```
  </Tab>
  
  <Tab title="Isolation Problems">
    Without proper isolation, you can experience:
    
    **1. Dirty Read**: Reading uncommitted data
    ```
    T1: UPDATE products SET price = 100 (not committed)
    T2: SELECT price → 100 (reads uncommitted change)
    T1: ROLLBACK
    T2: Now has invalid data!
    ```
    
    **2. Non-Repeatable Read**: Same query, different results
    ```
    T1: SELECT price → 100
    T2: UPDATE products SET price = 150; COMMIT
    T1: SELECT price → 150 (different!)
    ```
    ...
    **3. Phantom Read**: New rows appear
    ...
    T1: SELECT COUNT(*) WHERE status='pending' → 11 (phantom row!)
    ```
    ...
    **4. Lost Update**: Concurrent updates overwrite each other
    ...
    T2: Write balance = 100 + 30 = 130 (T1's update lost!)
    ```
  </Tab>
</Tabs>
...
<Tabs>
  <Tab title="Concept">
    **Durability** guarantees that once a transaction is committed, it survives any subsequent failures (crashes, power outages, etc.).
    ...
    │       ▼                                                         │
    │  ┌─────────────┐      ┌─────────────┐                          │
    │  │  WAL Buffer │ ───▶ │  WAL Disk   │  ← Durable!              │
    │  └─────────────┘      └─────────────┘                          │
    │       │                     │                                   │
    │       │ (async)             │ (crash recovery)                  │
    │       ▼                     ▼                                   │
    │  ┌─────────────┐      ┌─────────────┐                          │
    │  │ Data Buffer │ ───▶ │ Data Files  │                          │
    │  └─────────────┘      └─────────────┘                          │
    │                                                                 │
    │  Once WAL is on disk, transaction is DURABLE                   │
    │  even if data files haven't been updated yet                    │
    │                                                                 │
    └─────────────────────────────────────────────────────────────────┘
    ```
  </Tab>
  
  <Tab title="Durability Configuration">
    PostgreSQL provides configuration for durability vs. performance:
    
    ```sql
    -- Maximum durability (default)
    SET synchronous_commit = on;
    -- Waits for WAL to be written to disk before returning
    
    -- Higher performance, slight durability risk
    SET synchronous_commit = off;
    -- Returns immediately, WAL written asynchronously
    -- Risk: lose last few milliseconds of transactions on crash
    ...
    **When to relax durability**:
    ...
    - Caching tables
    - Logging where small gaps are acceptable
    
    **Never relax for**:
    - Financial transactions
    - User data
    - Anything that can't be reconstructed
  </Tab>
</Tabs>

---

## 3.3 Isolation Levels Explained

SQL defines four isolation levels, each preventing different concurrency problems.

<Frame>
  <img src="/images/courses/database-engineering/transaction-isolation.svg" alt="Transaction Isolation Levels" />
</Frame>
