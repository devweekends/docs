...
<AccordionGroup>
  <Accordion title="Explain the clock-sweep algorithm PostgreSQL uses for buffer eviction. Why was it chosen over true LRU?">
    **Strong Answer:**

    - Clock-sweep is an approximation of LRU that avoids the overhead of maintaining a strict ordering. In a true LRU, every buffer access requires moving the accessed buffer to the head of a linked list -- under high concurrency, this linked list becomes a contention point because every read/write must update it under a lock.
    - Clock-sweep works differently: each buffer has a usage_count (0-5). When a buffer is accessed, its usage_count is incremented (up to a cap of 5). When the system needs to evict a buffer, a "clock hand" sweeps circularly through the buffer pool. For each buffer: if usage_count > 0 and the buffer is not pinned, decrement usage_count and move on. If usage_count = 0 and unpinned, evict this buffer (the "victim"). Frequently accessed buffers survive multiple sweeps because their usage_count is repeatedly replenished.
    - Why it was chosen: (1) No global data structure to update on every access -- incrementing usage_count is a local atomic operation, minimal contention. (2) It naturally adapts to workload changes: hot pages accumulate high usage_count, cold pages drop to 0 quickly. (3) It resists sequential scan pollution: a full table scan touches every page once (usage_count = 1), which decays quickly, preserving genuinely hot pages in the pool.

    **Follow-up: What are ring buffers and when does PostgreSQL use them instead of the main buffer pool?**

    Ring buffers are small, dedicated buffer pools (typically 256KB to 16MB) used for bulk operations that would otherwise pollute the main shared buffer pool. Sequential scans on large tables, VACUUM, and bulk COPY operations use ring buffers. Without ring buffers, a sequential scan of a 100GB table would evict every other page from shared_buffers, destroying the cache for all other queries. With ring buffers, the scan reuses a small rotating set of buffers, leaving the main pool untouched. This is why a large sequential scan's `Buffers: shared read` count can exceed the number of shared_buffers without thrashing the cache.
  </Accordion>

  <Accordion title="How would you diagnose and resolve a buffer pool that is too small for the workload?">
    **Strong Answer:**

    - **Diagnosis**: Check the buffer cache hit ratio: `SELECT sum(blks_hit)::float / nullif(sum(blks_hit + blks_read), 0) FROM pg_stat_database`. For OLTP, below 99% is concerning. Below 95% is critical. Also check per-table: `SELECT relname, heap_blks_hit, heap_blks_read, round(heap_blks_hit::numeric / nullif(heap_blks_hit + heap_blks_read, 0), 4) as ratio FROM pg_statio_user_tables ORDER BY heap_blks_read DESC LIMIT 10`. This identifies which tables are causing the most disk reads.
    - **Deep diagnosis with pg_buffercache**: Install the pg_buffercache extension and check what is in the buffer pool: `SELECT c.relname, count(*) as buffers, pg_size_pretty(count(*) * 8192) as size FROM pg_buffercache b JOIN pg_class c ON b.relfilenode = c.relfilenode WHERE b.reldatabase = (SELECT oid FROM pg_database WHERE datname = current_database()) GROUP BY c.relname ORDER BY count(*) DESC LIMIT 20`. If a single large table dominates the pool and evicts other critical tables, you have a working set problem.
    - **Resolution options**: (1) Increase shared_buffers if RAM permits (but never beyond ~40% of total RAM). (2) If one table dominates, consider whether it truly needs to be fully cached -- perhaps partition it so only hot partitions are cached. (3) Add indexes to convert sequential scans (which read entire tables) into index scans (which read only needed pages). (4) Use pg_prewarm to pre-load critical tables after a restart. (5) If the working set genuinely exceeds available RAM, the answer is not more cache but less data -- archive old data, use summary tables for aggregations, or shard.

    **Follow-up: What is the relationship between shared_buffers and the OS page cache?**

    PostgreSQL uses double-buffering: when a page is read from disk, it exists in both the OS page cache and in shared_buffers. This sounds wasteful but is intentional. The OS cache acts as a larger, less-controlled second-level cache. effective_cache_size (typically 75% of RAM) tells the planner about the total effective cache (shared_buffers + estimated OS cache) without actually allocating it. This influences the planner's preference for index scans (which benefit from cached random reads) vs sequential scans. Setting shared_buffers too high starves the OS cache, often reducing total effective cache.
  </Accordion>

  <Accordion title="A bulk data load is destroying cache performance for your OLTP queries. What is happening and how do you fix it?">
    **Strong Answer:**

    - The bulk load (COPY, large INSERT batch, or sequential scan for ETL) is reading/writing large volumes of data through the buffer pool, evicting the hot OLTP pages. Even though PostgreSQL uses ring buffers for sequential scans, COPY operations and large batch inserts go through the main buffer pool.
    - **Immediate fix**: Run the bulk load during off-peak hours. If that is not possible, use an UNLOGGED table as a staging area (bypasses WAL and buffer pool contention), then INSERT INTO production_table SELECT FROM staging_table in smaller batches.
    - **Architectural fix**: (1) Dedicate a separate replica for ETL/analytics workloads so the primary's buffer pool is not contaminated. (2) If the bulk load must run on the primary, use `SET maintenance_work_mem = '2GB'` and `SET synchronous_commit = off` for the loading session to minimize I/O contention with OLTP queries. (3) After the load completes, run pg_prewarm on critical OLTP tables to reload them into the buffer pool.
    - **pg_prewarm example**: `SELECT pg_prewarm('critical_table', 'buffer', 'main')` loads the table's main fork into shared_buffers. For large tables, specify a block range: `SELECT pg_prewarm('critical_table', 'buffer', 'main', 0, 10000)` to load only the first 10000 blocks (80MB).

    **Follow-up: How does the background writer help in this scenario?**

    The background writer proactively writes dirty buffers to disk during quiet periods, freeing up clean buffers for reuse without forcing backends to do synchronous writes. During a bulk load, the background writer helps by gradually flushing dirty pages, reducing the chance that an OLTP query must wait for a dirty eviction (which requires writing the dirty page to disk before the buffer can be reused). Tuning `bgwriter_lru_maxpages` and `bgwriter_lru_multiplier` upward during bulk loads can help keep more clean buffers available.
  </Accordion>
</AccordionGroup>
