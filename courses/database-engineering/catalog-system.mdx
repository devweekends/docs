...
<AccordionGroup>
  <Accordion title="What is the syscache in PostgreSQL, and why does cache invalidation in the catalog system matter for production performance?">
    **Strong Answer:**

    - The syscache (system cache) is an in-process, per-backend cache of frequently accessed system catalog rows. Every time PostgreSQL needs to look up a table definition (pg_class), column info (pg_attribute), or function signature (pg_proc), it checks the syscache first instead of querying the catalog tables on disk. Without syscache, every SQL statement would require multiple catalog table lookups -- a SELECT on a 5-column table would hit pg_class, pg_attribute (5 times), pg_type, and potentially pg_index.
    - Cache invalidation is critical because catalog changes (DDL like ALTER TABLE, CREATE INDEX) must propagate to all backends. PostgreSQL uses a shared invalidation message queue: when a backend modifies a catalog entry, it posts an invalidation message. Other backends check this queue at the start of each transaction and evict stale syscache entries.
    - Production impact: (1) Running frequent DDL (e.g., creating/dropping temp tables in a loop) generates a flood of invalidation messages, causing all backends to flush and rebuild parts of their syscache. This is a known performance problem with ORMs that create temp tables per request. (2) The relcache (relation cache, a higher-level cache built on syscache) stores full relation descriptors including all columns, constraints, and indexes. Invalidating a relcache entry for a large table with many indexes forces an expensive rebuild.
    - The practical lesson: avoid DDL in hot paths. Creating a temp table per request is much more expensive than it appears because of the catalog invalidation cascade.

    **Follow-up: How does this affect connection pooling with PgBouncer?**

    In transaction mode, PgBouncer reassigns server connections between transactions. Each new transaction inherits the backend's syscache state from the previous user. This is normally fine, but if one user creates a temp table and another user gets the same backend, the second user's queries may trigger invalidation processing for the temp table's catalog entries. Worse, SET commands that change session state do not persist across PgBouncer transaction boundaries. This is why statement mode (where connections are released after each statement) requires stateless queries -- no SET, no temp tables, no prepared statements.
  </Accordion>

  <Accordion title="How would you use PostgreSQL system catalogs to diagnose a performance problem in production?">
    **Strong Answer:**

    - The catalog views (pg_stat_* family) are the primary diagnostic tool for PostgreSQL performance. Key queries:
    - **Slow queries**: `pg_stat_statements` (requires extension) shows every query fingerprint with call count, mean time, total time, and rows. Sort by total_exec_time to find the queries consuming the most database time.
    - **Table health**: `pg_stat_user_tables` shows seq_scan vs idx_scan counts (high seq_scan on large tables = missing index), n_dead_tup (bloat indicator), and last_autovacuum timing.
    - **Index usage**: `pg_stat_user_indexes` with idx_scan = 0 identifies unused indexes wasting write performance. `pg_statio_user_indexes` shows buffer hit ratios per index.
    - **Lock analysis**: `pg_locks` joined with `pg_stat_activity` reveals lock contention -- which queries are waiting and which are blocking.
    - **Connection state**: `pg_stat_activity` shows every backend's state (active, idle, idle in transaction), current query, and wait events. `state = 'idle in transaction'` for more than a few seconds is a red flag.
    - The meta-skill is knowing which catalog view answers which question. I have a saved set of diagnostic queries that I run in the first 60 seconds of any incident investigation.

    **Follow-up: What is the difference between pg_class.reltuples and an actual COUNT(*)?**

    `pg_class.reltuples` is an estimate updated by ANALYZE and autovacuum. It is not transactionally consistent -- it is a snapshot from the last statistics collection. `COUNT(*)` scans the entire table and returns an exact count at the current snapshot, but it is expensive (requires a full sequential scan on unindexed tables). For dashboards showing approximate row counts, use `reltuples`. For exact counts in application logic, use `COUNT(*)` with appropriate filters. For very large tables where even filtered counts are slow, maintain a counter table updated by triggers.
  </Accordion>

  <Accordion title="Explain what pg_class, pg_attribute, and pg_type are. How are they related, and why should a senior engineer understand them?">
    **Strong Answer:**

    - **pg_class**: Describes every "relation" in PostgreSQL -- tables, indexes, views, materialized views, sequences, composite types, TOAST tables. Each row represents one relation with its OID, name, namespace (schema), access method, storage parameters, and approximate row/page counts.
    - **pg_attribute**: Describes every column of every relation. Each row maps to one column with its data type (via pg_type OID), position (attnum), null-ability, default expression, and storage strategy (PLAIN, EXTENDED, EXTERNAL, MAIN for TOAST).
    - **pg_type**: Describes every data type -- built-in (integer, text, jsonb) and user-defined (enums, composite types, domain types). Each entry defines the type's input/output functions, storage size, alignment requirements, and category.
    - Relationships: pg_class.oid joins to pg_attribute.attrelid (columns of a table), pg_attribute.atttypid joins to pg_type.oid (data type of a column), and pg_class.reltype joins to pg_type.oid (the composite type representing a row of the table).
    - Why senior engineers must understand this: (1) Extension development requires inserting/modifying catalog entries. (2) Performance debugging uses catalog stats (pg_class.relpages, reltuples). (3) Schema introspection queries for tooling and migrations query these catalogs directly. (4) Understanding the catalog is prerequisite to reading PostgreSQL source code -- the catalog is the bridge between SQL concepts and internal data structures.

    **Follow-up: What happens in the catalog when you run ALTER TABLE ADD COLUMN?**

    A new row is inserted into pg_attribute for the new column. pg_class is updated (incremented version). If the column has a DEFAULT with a volatile expression, a pg_attrdef entry is added. If the default is a constant and the PostgreSQL version supports it (PG 11+), the default is stored in pg_attribute.attmissingval and no table rewrite occurs -- existing rows return the default without physically storing it. This is why ADD COLUMN with a constant default is instant in modern PostgreSQL regardless of table size.
  </Accordion>
</AccordionGroup>
