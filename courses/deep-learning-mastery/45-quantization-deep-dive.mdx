...
Think of it like a piano: FP32 is a piano with thousands of keys spanning the full audible range. INT8 is a piano with only 256 keys. Quantization decides which 256 pitches to include. A good quantization scheme puts more keys where the music actually plays (near the weight distribution's center) and fewer keys in the rarely-used extremes.
