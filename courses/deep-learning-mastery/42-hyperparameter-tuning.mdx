...
Hyperparameters are the knobs you set *before* training begins -- they control how the model learns, but they are not learned by gradient descent. Think of them as the recipe parameters for baking: oven temperature, baking time, and flour-to-sugar ratio are "hyperparameters" that you choose before baking starts. You can not adjust the oven temperature by tasting the dough mid-bake.

| Category | Examples | Typical Impact |
|----------|----------|----------------|
| **Architecture** | Layers, hidden sizes, attention heads | Medium -- defines capacity |
| **Optimization** | Learning rate, batch size, optimizer | **High** -- learning rate is almost always the most important |
| **Regularization** | Dropout, weight decay, augmentation | Medium -- prevents overfitting |
| **Training** | Epochs, warmup steps, gradient clipping | Low-Medium -- mostly about stability |

The search space is often **exponential**. If you have 5 hyperparameters each with 10 possible values, that is 100,000 combinations. At 30 minutes per training run, exhaustive search would take 5.7 years. How do we find good configurations efficiently?
...
class GridSearch:
    ...
    Pros: Simple, reproducible, complete coverage, easy to parallelize
    Cons: Exponential cost (curse of dimensionality), wastes budget on
          unimportant parameters
    
    When to use: When you have 2-3 hyperparameters and a small number of
    values each (< 100 total combinations). Beyond that, random search
    is almost always a better use of your compute budget.
    """
...
class RandomSearch:
    ...
    Why random beats grid: In most problems, only 1-2 hyperparameters
    actually matter. Grid search allocates equal budget to all parameters,
    wasting trials on unimportant ones. Random search ensures you sample
    many distinct values of every parameter, even the important ones.
    
    Analogy: If you are looking for a needle (optimal learning rate) in a
    haystack, grid search checks a regular pattern of points. Random search
    scatters darts -- and each dart has a unique learning rate value,
    giving you much better coverage of the dimension that matters.
    """
...
class GaussianProcess:
    ...
    Uses RBF kernel for surrogate modeling. A GP gives us not just a
    prediction of performance for untried configurations, but also an
    uncertainty estimate -- which is crucial for balancing exploration
    (trying uncertain regions) vs exploitation (trying regions we think
    are good).
    """
...
class PopulationBasedTraining:
    ...
    Key ideas:
    1. Train population of models in parallel (each with different hyperparams)
    2. Periodically exploit (poorly performing models copy the weights and
       hyperparameters of the best models) and explore (perturb hyperparameters
       slightly to discover new good settings)
    3. Adapts hyperparameters during training -- this is unique and powerful
    
    Practical tip: PBT is most valuable when the optimal hyperparameters
    change during training (e.g., learning rate schedules, augmentation
    strategies). For static hyperparameters, Bayesian optimization suffices.
    """
...
class SuccessiveHalving:
    ...
    Idea: Start many configurations with a small budget (few epochs), then
    keep only the top 1/eta performers and give them more budget. Repeat
    until one configuration remains.
    
    Analogy: It is like a tournament bracket -- the worst performers are
    eliminated each round, and survivors get more training time.
    
    Pitfall: If a configuration needs many epochs to "warm up" (common with
    large learning rates or complex schedules), it might get unfairly
    eliminated early. Hyperband (below) addresses this by running multiple
    brackets with different starting budgets.
    """
