...
class DepthwiseSeparableConv(nn.Module):
    ...
    def __init__(
        ...
        # Depthwise: each input channel gets its own independent spatial filter.
        # Think of it as asking each channel "what spatial pattern do you see?"
        # without any cross-channel communication yet.
        # The magic: groups=in_channels means each channel has its OWN filter.
        self.depthwise = nn.Conv2d(
            ...
            stride=stride,
            padding=padding,
            groups=in_channels,  # Key: groups = in_channels (one filter per channel)
            bias=bias
        )
        
        # Pointwise: 1x1 conv for channel mixing.
        # Now we let channels talk to each other: "given what each channel found
        # spatially, how should we combine that information?"
        # This is just a learned linear combination across the channel dimension.
...
class InvertedResidual(nn.Module):
    ...
    1. Inverted bottleneck: expand -> depthwise -> project
       (Traditional ResNet bottleneck: wide -> narrow -> wide.
        MobileNetV2 inverts this: narrow -> wide -> narrow.
        Why? The heavy depthwise conv runs in the expanded high-dim space
        where it can capture richer patterns, then projects back down.)
    2. Linear bottleneck: no ReLU after final projection
       (ReLU destroys information in low-dimensional spaces by zeroing
        negative values. Removing it in the narrow bottleneck preserves
...
class ShuffleNetV2(nn.Module):
    ...
    G3: Network fragmentation reduces parallelism (small ops underutilize GPU cores)
    G4: Element-wise operations are non-negligible (ReLU, skip add, etc.)
    
    These guidelines are derived from actual hardware profiling, not theoretical
    FLOP counting. This is why ShuffleNetV2 is often faster in practice than
    models with lower theoretical FLOPs.
    """
...
class EfficientNetConfig:
    ...
    Previous approaches would scale only one dimension (e.g., make the
    network deeper OR wider). EfficientNet showed that scaling all three
    together with a fixed ratio gives much better accuracy per FLOP.
    
    Analogy: If you are enlarging a photograph, you scale both width and
    height proportionally. Stretching only one dimension produces a distorted
    image. Same logic applies to neural network scaling.
    
    phi: compound coefficient (the single knob you turn)
    depth = alpha^phi
    width = beta^phi  
    resolution = gamma^phi
    
    Subject to: alpha x beta^2 x gamma^2 approx 2 (FLOPs roughly double per step)
    """
...
class GhostModule(nn.Module):
    ...
    Idea: Many feature maps are similar (redundant). If you visualize
    intermediate feature maps of a trained CNN, you will notice many look
    like slight variations of each other -- rotated, shifted, or scaled
    versions. GhostNet exploits this: generate a few "intrinsic" features
    with a standard convolution, then create "ghost" copies cheaply using
    simple depthwise transforms. It is like photocopying a document and
    making small edits instead of writing each copy from scratch.
    """
