...
The shift from RNNs to Transformers is analogous to the shift from sequential file processing to databases with indexes. An RNN reads data one record at a time, building up context linearly. A Transformer can "query" any part of the sequence directly -- like a database lookup -- making it dramatically more parallelizable and better at long-range dependencies.
</Note>
...
class PositionwiseFeedForward(nn.Module):
    ...
    Typically expands dimension by 4x (d_ff = 4 * d_model), then projects back.
    
    Why does this exist? Attention is a linear weighted average of values --
    by itself, it can only compute linear combinations. The FFN adds the
    non-linear processing power that lets the transformer learn complex
    transformations. Think of attention as "gathering information" and
    FFN as "processing that information."
    """
    
    def __init__(self, d_model, d_ff, dropout=0.1):
        super().__init__()
        
        self.linear1 = nn.Linear(d_model, d_ff)    # Expand: d_model -> 4*d_model
        self.linear2 = nn.Linear(d_ff, d_model)     # Compress back: 4*d_model -> d_model
        self.dropout = nn.Dropout(dropout)
...
class LayerNorm(nn.Module):
    ...
    More stable than batch norm for variable-length sequences because
    it does not depend on batch statistics -- each sample is normalized
    independently, so batch size 1 works fine (critical for inference).
    
    LayerNorm(x) = gamma * (x - mean) / (std + eps) + beta
    
    gamma and beta are learnable parameters that allow the network
    to undo the normalization if that turns out to be useful. Without them,
    normalization would permanently constrain the representation.
    """
...
Think of positional encoding like a clock with many hands of different speeds. The second hand completes a full cycle every 60 seconds, the minute hand every 60 minutes, the hour hand every 12 hours. At any moment, the combination of all hand positions uniquely identifies the time. Sinusoidal positional encoding does exactly the same thing: dimension 0 oscillates rapidly (like the second hand, completing a cycle every few positions), dimension 50 oscillates slowly (like the minute hand), and dimension 500 oscillates very slowly (like the hour hand). The combination of all these "hands" at a given position creates a unique fingerprint that the model can learn to decode.

**Mathematical intuition**: The frequencies decrease geometrically from $1$ (for the first dimension pair) to $1/10000$ (for the last dimension pair). This gives the model both fine-grained position information (nearby positions differ in high-frequency dimensions) and coarse-grained information (distant positions differ in low-frequency dimensions). The key mathematical property is that for any fixed offset $k$, the encoding at position $pos+k$ can be expressed as a linear transformation of the encoding at position $pos$ -- specifically, a rotation matrix applied to each (sin, cos) pair. This means the model can learn relative position relationships using simple linear operations.
...
class PositionalEncoding(nn.Module):
    ...
    The problem: self-attention is permutation-invariant -- it treats
    "the cat sat on the mat" identically to "mat the on sat cat the."
    We need to inject position information somehow.
    
    The solution: add sin/cos signals of different frequencies to the embeddings.
    Each position gets a unique "fingerprint" of sine and cosine values.
    
    Why sin/cos? Two key properties:
    1. The encoding for position p+k can be expressed as a linear function
       of the encoding for position p, which helps the model learn relative positions.
    2. The pattern extends to sequence lengths not seen during training (extrapolation).
    """
...
class EncoderLayer(nn.Module):
    ...
    Two critical design patterns here:
    1. Residual connections (Add): let gradients flow directly through,
       preventing vanishing gradients in deep stacks (same idea as ResNet).
    2. Layer normalization (Norm): stabilizes the activation distribution
       after each sub-layer, enabling higher learning rates and faster convergence.
    """
...
class Transformer(nn.Module):
    ...
    def make_src_mask(self, src, pad_idx=0):
        """Create source padding mask -- prevents attention to <PAD> tokens."""
        # Shape: (batch, 1, 1, src_len) -- broadcastable across heads and queries
        src_mask = (src != pad_idx).unsqueeze(1).unsqueeze(2)
        return src_mask
    
    def make_tgt_mask(self, tgt, pad_idx=0):
        """
        Create target mask combining TWO constraints:
        1. Padding mask: don't attend to <PAD> tokens
        2. Causal mask: don't attend to future tokens (no "peeking ahead")
        
        The causal mask is what makes the decoder autoregressive -- when
        predicting word t, it can only see words 1..t-1. Without this,
        the model would trivially copy the answer from the future.
        """
        batch_size, tgt_len = tgt.shape
        
        # Padding mask: True where token is real, False where <PAD>
        pad_mask = (tgt != pad_idx).unsqueeze(1).unsqueeze(2)
        
        # Causal (lower-triangular) mask: position i can only attend to positions <= i
    ...
    def generate(self, src, max_len=50, start_token=1, end_token=2):
        ...
        This is the inference loop: the model predicts one token, appends it
        to the output, and uses the extended output to predict the next token.
        This is inherently sequential -- you cannot parallelize generation
        across time steps (each step depends on the previous one).
        
        This sequential bottleneck is why training is fast (all positions
        in parallel via teacher forcing) but inference is slow (one token
        at a time). It is also why techniques like speculative decoding
        and KV-caching exist: to speed up this serial generation process.
