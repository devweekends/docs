...
Reading and writing binary data is essential for systems programming -- from file formats to network protocols. Every PNG image, every SQLite database, every TCP packet you have ever interacted with is binary data structured according to precise rules. Learn to handle endianness, struct packing, and serialization correctly, and you will understand how software really communicates with itself and the outside world.
...
int write_records(const char *filename, const struct Record *records, size_t count) {
    FILE *fp = fopen(filename, "wb");  // "wb" = write binary (not "w" -- text mode mangles bytes on Windows)
    if (!fp) {
        perror("fopen");
        return -1;
    }
    
    // Write header -- magic number lets readers quickly reject non-RECD files
    uint32_t magic = 0x52454344;  // "RECD" in ASCII
    uint32_t version = 1;         // Versioning allows format evolution without breaking old readers
    uint32_t num_records = (uint32_t)count;
    
    // WARNING: fwrite dumps raw bytes from memory. The on-disk format is now
    // tied to this machine's endianness and struct padding. This is fine for
    // single-machine use but NOT portable across architectures. See the
    // "Portable Serialization" section below for the cross-platform approach.
...
int read_records(const char *filename, struct Record **records, size_t *count) {
    ...
    // PITFALL: Always validate num_records before multiplying. A malicious file
    // could set num_records to UINT32_MAX, causing the multiplication to overflow
    // and malloc to allocate a tiny buffer, leading to a heap buffer overflow.
    if (num_records > 1000000) {  // Sanity check: reject unreasonable sizes
...
Think of endianness like writing a phone number. In some countries, you write the area code first (most significant part first -- big-endian). In others, you start with the local digits (least significant part first -- little-endian). Both represent the same number, but if two people use different conventions and do not agree on which one to use, the number they read will be wrong. This is exactly what happens when a big-endian machine writes a file and a little-endian machine reads it without conversion.
...
The CPU is like a forklift that can only pick up items at specific positions on a shelf. A 4-byte integer must sit at an address divisible by 4 (its "natural alignment"). The compiler inserts invisible padding bytes between struct fields to satisfy these alignment constraints. This makes in-memory access fast but means that `sizeof(struct)` is often larger than the sum of its fields -- and critically, it means you cannot just write a struct to disk and expect another machine (or even another compiler) to read it back correctly.
...
The chunk-based format is the most successful binary format design pattern in computing. PNG, WAV (RIFF), AVI, IFF, WebP, and TIFF all use it. The key insight: each chunk has a type tag and a length, so a reader can skip any chunk it does not understand. This means you can add new chunk types in version 2 without breaking version 1 readers -- they just skip the unknown chunks. This forward-compatibility property is why the format has survived for decades.
...
Memory-mapped I/O lets you treat a file as if it were an array in memory. Instead of calling `read()` and `write()` to copy data between the kernel and your buffer, `mmap` maps the file's pages directly into your process's virtual address space. The OS handles paging data in and out transparently. This is how databases like SQLite and PostgreSQL access their data files -- it avoids the overhead of explicit system calls for every read and lets the kernel's page cache do the heavy lifting.
...
void *map_file(const char *filename, size_t *size, int writable) {
    ...
    *size = st.st_size;
    ...
    close(fd);  // Safe to close fd after mmap -- the mapping keeps its own reference
...
int64_t find_pattern(const char *filename, const uint8_t *pattern, size_t pattern_len) {
    ...
    // Tell the kernel we will read sequentially so it can prefetch pages ahead.
    // Without this hint, the kernel assumes random access and prefetches less
    // aggressively, which can make sequential scans 2-3x slower on large files.
...
Serialization is the process of converting in-memory data structures into a stream of bytes that can be stored or transmitted. The challenge is that the in-memory representation (with pointers, padding, platform-specific sizes) does not survive crossing machine boundaries. A proper serializer writes each field in a defined byte order, with explicit sizes, using no pointers -- only offsets or lengths. This is what protocol buffers, MessagePack, and FlatBuffers do under the hood.
