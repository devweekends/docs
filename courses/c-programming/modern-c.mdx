...
C continues to evolve. Modern C (C11, C17, C23) adds powerful features for type safety, concurrency, and expressiveness while maintaining backward compatibility. If C89 gave you a hammer and a saw, modern C hands you a level and a tape measure -- the core job is the same, but the tools for catching mistakes before they ship have gotten dramatically better.
...
// Think of _Static_assert as a contract with the compiler: "I am building on
// these assumptions. If any of them are wrong, REFUSE TO COMPILE." This catches
// portability bugs that would otherwise silently produce wrong results on a
// different architecture -- the kind of bug you discover six months later when
// someone runs your code on a 16-bit microcontroller or a big-endian mainframe.
...
`_Generic` is C's answer to function overloading. It selects an expression at compile time based on the type of its first argument. This is how `<tgmath.h>` provides type-generic math functions (e.g., `sin()` works on `float`, `double`, and `long double` without the caller specifying which). It is also the key building block for type-safe macros that do not sacrifice performance.
...
Atomics let multiple threads read and write shared variables without locks -- but only for simple operations. Think of an atomic operation like a turnstile at a subway station: no matter how many people push through simultaneously, the counter always increments by exactly one per person. The hardware guarantees indivisibility. But if your "operation" requires reading the counter, doing math, and writing it back as a multi-step transaction, you still need a lock (or a compare-and-swap loop).
...
// PITFALL: Spinlocks burn CPU cycles while waiting. They are only appropriate
// when the critical section is extremely short (a few instructions) and
// contention is rare. For anything longer, use a mutex -- spinning for
// milliseconds wastes energy and starves other threads of CPU time.
atomic_flag lock = ATOMIC_FLAG_INIT;

void spinlock_acquire(atomic_flag *lock) {
    while (atomic_flag_test_and_set_explicit(lock, memory_order_acquire)) {
        // Spin -- in production, add a pause/yield hint here to reduce
        // pipeline stalls and power consumption on x86: __builtin_ia32_pause()
    }
}
...
// Memory ordering options -- this is the hardest part of atomics.
// The compiler and CPU both reorder instructions for performance. Memory
// orderings constrain HOW MUCH reordering is allowed around an atomic op.
// 
// Think of it like a fence in a garden: operations on one side cannot
// "leak" past the fence to the other side.
//
// memory_order_relaxed  - No fence at all. Only guarantees atomicity of THIS
//                         operation. Other threads may see stores in any order.
//                         Use for: counters where you only care about the final value.
// memory_order_acquire  - "Read fence": nothing below this line moves above it.
//                         Use on: loads that guard access to shared data.
// memory_order_release  - "Write fence": nothing above this line moves below it.
//                         Use on: stores that publish shared data.
// memory_order_acq_rel  - Both fences. Use on: read-modify-write ops (CAS).
// memory_order_seq_cst  - Full sequential consistency (default, safest, slowest).
//                         All threads agree on a single total order of operations.
//                         Use when: you are unsure which ordering to pick.
//
// PITFALL: If you do not understand memory ordering, use the default (seq_cst).
// Incorrect relaxed orderings cause bugs that only manifest under high load on
// weakly-ordered CPUs (ARM, RISC-V) and are nearly impossible to reproduce.
...
<Warning>
**Atomics are not a universal replacement for locks.** They work well for single-variable updates (counters, flags, pointers). But if you need to update two variables atomically (e.g., a pointer and a length), you need a mutex. Trying to build multi-variable consistency with atomics alone leads to subtle, intermittent bugs that only appear under high contention on multi-core machines.
</Warning>

### C11 Threads (`<threads.h>`)

C11 introduced a standard threading library so that portable C code does not have to depend on platform-specific APIs like POSIX pthreads or Windows threads. In practice, many projects still use pthreads because `<threads.h>` support was slow to arrive in compilers (MSVC only added partial support recently, and some embedded toolchains still lack it). When writing new portable code, prefer `<threads.h>` if your target compilers support it; otherwise, wrap pthreads behind a thin abstraction layer.
...
This might seem like a minor change, but it fixes a real class of bugs. In pre-C23 code, `NULL` is a macro that expands to either `0` or `(void*)0` depending on the implementation. Since `0` is a valid integer, passing `NULL` to a variadic function or a `_Generic` selector can silently pick the wrong branch. `nullptr` eliminates this ambiguity by being unambiguously a pointer type.
...
Before C23, the only way to define a compile-time constant in C was `#define` (which has no type and no scope) or `const` (which is not actually a compile-time constant in C -- unlike C++, you cannot use a `const int` as an array size in standard C). `constexpr` fills this gap: it creates a typed, scoped constant that is guaranteed to be evaluated at compile time.
...
**Practical note**: In C (unlike C++), `auto` has historically meant "automatic storage duration" (the default for local variables). C23 repurposes it for type inference, but only when an initializer is present. This means old code that explicitly wrote `auto int x = 5;` will change meaning under C23. In practice this was extremely rare, but be aware of it when compiling legacy codebases with `-std=c23`.
