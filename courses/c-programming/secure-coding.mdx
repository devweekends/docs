...
C's power comes with responsibility. Buffer overflows, format string attacks, and integer overflows have caused billions of dollars in damage. The Morris Worm (1988) exploited a buffer overflow in `fingerd`. The Heartbleed bug (2014) was a bounds-check failure in OpenSSL. The Cloudbleed incident (2017) was caused by a buffer over-read in an HTML parser written in C. These are not ancient history -- memory safety vulnerabilities in C code remain the #1 source of critical security bugs in 2025.

This module teaches you the defensive patterns that distinguish secure C code from code that is just waiting to be exploited.
...
The buffer overflow is the "hello world" of security vulnerabilities, and it is still the most common class of exploit in C code. The mechanism is simple: you write more data into a buffer than it can hold, and the excess overwrites whatever happens to be next in memory -- typically the function's return address on the stack.
...
void vulnerable_login(const char *username) {
    char buffer[16];
    strcpy(buffer, username);  // No bounds check! Copies until it hits '\0'.
    printf("Welcome, %s\n", buffer);
}

// What happens with a 32-character username?
// Stack layout (growing downward):
// [buffer 16 bytes][saved RBP 8 bytes][return address 8 bytes]
//
// A 32-byte string overwrites the buffer (16 bytes), the saved frame pointer
// (next 8 bytes), and the return address (next 8 bytes). When the function
// returns, the CPU jumps to whatever address the attacker placed in those
// last 8 bytes. With careful crafting, this leads to arbitrary code execution.
//
// Modern mitigations (ASLR, stack canaries, NX bit) make exploitation harder
// but NOT impossible. The only reliable defense is to prevent the overflow.
```
...
Integer overflow vulnerabilities are more subtle than buffer overflows because the overflow happens in an arithmetic operation, not a memory copy. The attacker provides input that causes a size calculation to wrap around, resulting in a much smaller allocation than intended. The program then writes the "intended" amount of data into the too-small buffer -- a buffer overflow triggered by arithmetic rather than string copying.
...
void *vulnerable_alloc(size_t count, size_t size) {
    return malloc(count * size);  // Multiplication can overflow to a small value!
}

// Attack scenario on a 32-bit system:
// count = 0x100000001 (4,294,967,297), size = 8
// Intended allocation: 0x800000008 (34 GB -- way too large)
// Actual allocation after 32-bit wrap: 8 bytes
// The program thinks it allocated space for billions of elements but only got 8 bytes.
// Writing "count * size" bytes into an 8-byte buffer = catastrophic heap overflow.
...
Every production C binary should be compiled with these flags. They are defense-in-depth: each flag mitigates a different class of attack, and together they make exploitation significantly harder.
...
#
# -fstack-protector-strong: Places a "canary" value between local buffers and
#   the return address. If a buffer overflow corrupts the canary, the program
#   aborts before the corrupted return address is used. "strong" protects any
#   function with local arrays or address-taken variables.
#
# -D_FORTIFY_SOURCE=2: Replaces unsafe functions (memcpy, strcpy, sprintf)
#   with bounds-checked versions when the compiler can determine the buffer
#   size at compile time. Zero runtime cost for safe calls, abort on overflow.
#
# -fPIE -pie: Compiles as a Position Independent Executable, enabling ASLR
#   (Address Space Layout Randomization). The binary loads at a random address
#   each run, so attackers cannot hardcode memory addresses in exploits.
#
# -Wl,-z,now: Resolves all dynamic symbols at load time instead of lazily.
#   This prevents GOT (Global Offset Table) overwrite attacks that exploit
#   the lazy binding mechanism to redirect function calls.
#
# -Wl,-z,relro: Makes the GOT and other relocation sections read-only after
#   loading. Combined with -z,now, this is called "full RELRO" and provides
#   strong protection against GOT overwrite attacks.
```
