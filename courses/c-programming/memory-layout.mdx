...
**The problem**: A flat memory model would be inefficient and insecure.
...
**Real-world impact**: This segmentation enables:
- Memory protection (can't execute data, can't write code)
- Efficient sharing (text segment shared across processes)
- Fast allocation (stack is bump-pointer, heap uses allocator)
...
**Examples**:
```c
// Stack: small, fixed size, local scope
void process() {
    int buffer[100];  // Stack - fast, automatic cleanup
    // ...
}  // buffer automatically freed

// Heap: large, variable size, or needs to outlive function
char* load_file(size_t size) {
    return malloc(size);  // Heap - size unknown, outlives function
}  // Caller must free

// Stack: temporary calculation
void calculate() {
    double matrix[10][10];  // 800 bytes - fine for stack
}

// Heap: large or persistent data
void process_image(int width, int height) {
    // width * height * 4 bytes - could be megabytes!
    uint8_t *pixels = malloc(width * height * 4);
}
```

**Common mistake**: Using stack for large buffers:
```c
void bad() {
    char huge[1024 * 1024];  // 1MB on stack - likely stack overflow!
}

void good() {
    char *huge = malloc(1024 * 1024);  // 1MB on heap - safe
    // ... use it ...
    free(huge);
}
```
...
<Frame>
  <img src="/images/courses/c-stack-frame.svg" alt="C Stack Frame Layout" />
</Frame>
