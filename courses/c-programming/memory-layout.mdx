...
<Frame>
  <img src="/images/courses/c-memory-layout.svg" alt="C Process Memory Layout" />
</Frame>
...
Before examining the memory layout, understand why it's organized this way:

**The problem**: A flat memory model would be inefficient and insecure. If code and data were mixed, a bug could overwrite instructions, or a hacker could inject malicious code.

**The solution**: Separate segments with different properties:
...
This diagram shows how a C program looks in memory (virtual address space) when it's running.
...
Before diving into stack mechanics, understand when to use stack vs heap:
...
When a function is called, a new "frame" is pushed onto the stack. This frame contains everything the function needs to run.

![Function Call Mechanism](/images/courses/c-function-call-mechanism.svg)
...
![C Stack Frame Layout](/images/courses/c-stack-frame.svg)
...
<AccordionGroup>
  <Accordion title="A process on a 64-bit Linux system has 4GB of heap allocated but RSS (Resident Set Size) shows only 200MB. How is this possible?">
    **Strong Answer:**
    ...
    **Follow-up:** How does ASLR (Address Space Layout Randomization) affect the memory layout, and why does it matter for security?
  ...
  <Accordion title="Why does the stack grow downward and the heap grow upward? What happens when they collide?">
    **Strong Answer:**
    ...
    **Follow-up:** How would you detect a stack overflow in a production system?
  ...
  <Accordion title="Explain how mmap works and when you would use it instead of malloc for large allocations.">
    **Strong Answer:**
    ...
    **Follow-up:** You are building a database engine that needs to access a 100GB file. How would you use mmap, and what are the pitfalls?
