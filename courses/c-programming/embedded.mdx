...
Embedded programming is like cooking in a tiny kitchen: every utensil has a fixed place, you cannot order more counter space at runtime, and if you make a mistake nobody is there to catch the falling plate. There is no operating system to catch your segfault, no debugger attached by default, and often no way to print a message. The constraints force a discipline that makes you a better programmer everywhere else.
...
// Unlike normal variables (which live in RAM and the compiler can cache in CPU
// registers), hardware registers can change value spontaneously (a button press,
// a timer tick, a DMA transfer completing). The 'volatile' qualifier tells the
// compiler: "read this from memory EVERY time, and write to it EVERY time --
// even if you think the value hasn't changed." Without volatile, the compiler
// will optimize away "redundant" reads, and your code will miss hardware events.
...
void SysTick_Handler(void) {
    tick_count++;  // Keep ISRs SHORT. While this handler runs, all lower-priority
    // interrupts are blocked. A long ISR means missed UART bytes, delayed timer
    // ticks, and unpredictable system behavior. Rule of thumb: set a flag or
    // enqueue data, then do the real work in the main loop.
}
...
static inline uint32_t disable_interrupts(void) {
    uint32_t primask;
    __asm__ volatile(
        "mrs %0, primask\n"  // Save current interrupt state (were they already disabled?)
        "cpsid i\n"           // Disable all interrupts
        : "=r"(primask)
    );
    return primask;  // Return saved state so we can restore it later
}

static inline void restore_interrupts(uint32_t primask) {
    // Restore the PREVIOUS interrupt state rather than blindly re-enabling.
    // This is critical: if interrupts were already disabled by a caller,
    // we must not re-enable them here (that would break the caller's
    // critical section). This pattern enables nested critical sections.
