...
If a regular pointer is a sticky note with a memory address where data lives, a function pointer is a sticky note with the address where *code* lives. You can pass these addresses around, store them in arrays, embed them in structs, and call the code they point to -- all at runtime. This is how C programs implement the same flexibility that C++ gets from virtual functions, Java gets from interfaces, and Python gets from first-class functions. Under the hood, C++ vtables are literally arrays of function pointers.
...
Callbacks are the bread and butter of event-driven C code. The pattern is: "I don't know what you want to do when X happens, so give me a function pointer and I will call it for you." This is how `qsort` knows how to compare your custom structs, how signal handlers work, how GUI frameworks dispatch events, and how plugin systems extend functionality at runtime.
...
// Callback type. The void *user_data parameter is a critical pattern: it lets
// the callback access state without global variables. The caller passes context
// when registering, and the system passes it back when invoking. This is C's
// equivalent of a closure -- the function pointer is the code, and user_data
// is the captured environment. Every serious C API (pthreads, signal handlers,
// qsort_r, GTK) uses this pattern.
...
This is exactly how C++ polymorphism works under the hood. When you write `shape->draw()` in C++, the compiler generates code that follows a function pointer in a vtable -- the same pattern shown explicitly below. Understanding this demystifies inheritance, virtual functions, and the cost of polymorphism (one pointer indirection per virtual call).
...
// Virtual table structure -- one shared instance per "class" (not per object).
...
// Base "class" - all shapes have a vtable pointer as their first member.
// This pointer costs 8 bytes per object (on 64-bit) -- the same overhead
// C++ virtual classes pay for their hidden vptr.
typedef struct {
    const ShapeVTable *vtable;
    int x, y;  // Common fields
} Shape;

// Circle "class" -- base MUST be the first member so that a Circle* can be
// safely cast to a Shape*. This is guaranteed by the C standard: the address
// of a struct equals the address of its first member (no leading padding).
typedef struct {
...
// Compare integers.
// PITFALL: Never write "return *(int*)a - *(int*)b;" even though you will see
// it everywhere. If a = INT_MAX and b = -1, the subtraction overflows and
// produces undefined behavior. The pattern below is safe for all values.
int compare_int(const void *a, const void *b) {
    int ia = *(const int *)a;
    int ib = *(const int *)b;
    return (ia > ib) - (ia < ib);  // Returns -1, 0, or 1 without overflow risk
}
