...
Build a production-grade HTTP server that handles thousands of concurrent connections. You will implement the HTTP protocol, connection management, and high-performance I/O.

This project teaches you the architecture behind nginx, HAProxy, and every modern high-performance network server. The key insight: you cannot create one thread per connection (10,000 connections means 10,000 threads, each consuming ~8MB of stack space = 80GB of virtual memory). Instead, you use an event loop with `epoll` -- a single thread monitors thousands of sockets simultaneously and only processes the ones that have data ready. This is how nginx handles millions of connections on a single server.
...
http_server_t *server_create(int port, int max_connections) {
    ...
    // Socket options -- each one solves a specific production problem:
    int opt = 1;
    // SO_REUSEADDR: Allow bind() even if the port is in TIME_WAIT state from a
    // previous server instance. Without this, restarting the server immediately
    // after stopping it fails with "Address already in use" for ~60 seconds.
    setsockopt(server->listen_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
    // SO_REUSEPORT: Allow multiple server processes to bind to the same port.
    // The kernel load-balances incoming connections across them. Used by nginx
    // worker processes for zero-downtime restarts.
    setsockopt(server->listen_fd, SOL_SOCKET, SO_REUSEPORT, &opt, sizeof(opt));
    // TCP_NODELAY: Disable Nagle's algorithm, which buffers small writes to
    // combine them into larger TCP segments. For a request-response protocol,
    // Nagle adds latency (up to 40ms) with no throughput benefit.
    ...
    // EPOLLET = edge-triggered mode: epoll only notifies you when the state
    // CHANGES (new data arrives), not when data EXISTS. This means you must
    // read/accept in a loop until you get EAGAIN. Edge-triggered is more
    // efficient (fewer spurious wakeups) but requires correct loop handling.
    // The alternative, level-triggered (default), notifies as long as data
    // is available -- simpler but wakes the event loop more often.
    struct epoll_event ev = {
        .events = EPOLLIN | EPOLLET,
        .data.ptr = NULL              // NULL indicates this is the listener socket
    };
