...
**Analogy for Interval DP**: Think of a game show where contestants remove items from a line, earning points based on what is next to them. The order of removal matters. Interval DP captures this by asking: "what is the optimal way to handle items in positions i through j?" You try every possible "last action" for the interval and pick the best.

**Analogy for Bitmask DP**: Imagine assigning 15 tasks to 15 workers. Each assignment depends on who is already assigned. A bitmask tracks which workers are taken (bit i = 1 means worker i is assigned). With 2^15 = 32,768 possible assignment states, you can enumerate them all.
...
**Key Insight**: Think backwards--which balloon do we burst **last** in a range? This is the crucial mental shift. If we think about which balloon to burst first, the neighbors change after each burst, creating complex dependencies. But if we think about which balloon is burst last in (left, right), its neighbors are fixed: they are `left` and `right` (since everything in between has already been burst).

**Edge case**: When the range has no balloons between `left` and `right`, there is nothing to burst, so `dp[left][right] = 0`.
...
**When to Use**: n <= 20 (since 2^20 ~ 10^6). At n=20, you have about 10^6 states; at n=25, it is 33 million (borderline); at n=30, it is 10^9 (too much). Always check: 2^n * n (common complexity) must fit within the time limit.
...
When the DP transition involves a sum over a range (e.g., dp[i] = sum of dp[j] for l <= j <= r), precompute prefix sums of the DP array to make each transition O(1) instead of O(n).

### Monotonic Queue Optimization

When dp[i] = min/max(dp[j] + cost(j, i)) for j in a sliding window of size k. The key idea: maintain a deque of candidate j values. Remove candidates from the front when they fall out of the window, and remove from the back when a new candidate is better (they will never be used since dp[i] is already better and i is newer).

This reduces the per-transition cost from O(k) to amortized O(1), since each element is pushed and popped at most once.
