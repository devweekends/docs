...
<img src="/images/courses/cp/segment-tree-structure.svg" alt="Segment Tree Structure" />

## Why Segment Trees?

Segment trees answer **range queries** and handle **point/range updates** in O(log n) time. They are the single most important data structure in competitive programming after arrays and maps.

**Analogy**: Imagine a company org chart. The CEO at the top has aggregate sales for the entire company. Each VP has aggregate sales for their division. Each manager has totals for their team. To find sales for any range of employees, you combine a few pre-computed subtotals instead of summing individual numbers. A segment tree works the same way: it pre-computes answers for ranges of powers of 2, so any arbitrary range can be answered by combining O(log n) precomputed pieces.

Segment trees are essential for problems involving:
- Range sum/min/max/GCD queries
- Range updates (add/set all elements)
- Count elements in range satisfying condition
- Finding the first/last element in a range meeting a criterion
...
Change the combine function and identity element. The identity element is the value that, when combined with any value x, returns x (like 0 for addition, infinity for min, negative infinity for max).
...
long long identity = LLONG_MAX;  // min(x, INF) = x

// Range Maximum Query
long long combine(long long a, long long b) {
    return max(a, b);
}
long long identity = LLONG_MIN;  // max(x, -INF) = x

// Range GCD Query
long long combine(long long a, long long b) {
    return __gcd(a, b);
}
long long identity = 0;  // gcd(x, 0) = x

// Range XOR Query
long long combine(long long a, long long b) {
    return a ^ b;
}
long long identity = 0;  // x ^ 0 = x
```

<Tip>
**Contest tip**: The combine function must be **associative**: combine(a, combine(b, c)) == combine(combine(a, b), c). This is why "range second-minimum" or "range median" cannot be solved with a basic segment tree--they are not associative operations. For those, you need a merge sort tree or persistent segment tree.
</Tip>
...
## Pattern 4: Segment Tree on Indices (Walking the Tree)

**Problem**: Track positions where something is true.

**The technique**: Instead of querying a range and getting a value, you "walk" the tree to find a specific index satisfying some condition. The key is early termination: if a node's aggregate cannot satisfy the condition (e.g., the max in this range is <= x), skip the entire subtree.

**When to use**: "Find leftmost/rightmost index in [l, r] where value > x", "find kth one in a range," or "find first available slot." These queries are O(log n) because you descend the tree, visiting at most O(log n) nodes.

```cpp
// Find leftmost index in [l, r] where arr[i] > x
// tree[node] stores the MAX of the range -- if max <= x, no element qualifies
int findFirst(int node, int start, int end, int l, int r, int x) {
    if (r < start || end < l || tree[node] <= x) return -1;  // Prune: no qualifying element
    
    if (start == end) return start;  // Leaf: this is the answer
    
    int mid = (start + end) / 2;
    int left = findFirst(2 * node, start, mid, l, r, x);  // Check left child first (leftmost)
    if (left != -1) return left;  // Found in left subtree
    return findFirst(2 * node + 1, mid + 1, end, l, r, x);  // Otherwise check right
}
```
