...
These are the costs that do not show up in Azure's pricing calculator but appear on your bill. Every cloud engineer should audit for these monthly.

**1. Stopped VMs still cost money** (if not deallocated)
- VM stopped (OS shutdown): Still charges compute -- Azure reserves the physical hardware for you
- VM deallocated: Compute stops, only storage charged
- **How to tell the difference**: In the Portal, look for "Stopped (deallocated)" not just "Stopped". From CLI: `az vm get-instance-view --name myVM --resource-group myRG --query instanceView.statuses[1]`

**2. Unattached disks keep billing**
- Deleted VM, but disk still exists: $100/month waste per Premium P30 disk
- This happens because Azure defaults to "keep disk" when you delete a VM from the Portal
- **Fix**: Run `az disk list --query "[?managedBy==null].{Name:name, Size:diskSizeGb, RG:resourceGroup}"` monthly to find orphaned disks

**3. Snapshots are not free**
- 10 snapshots x 100 GB x $0.05/GB = $50/month
- Snapshots accumulate silently -- backup policies often create daily snapshots without cleanup
- **Fix**: Set up lifecycle policies to delete snapshots older than 30 days, or use Azure Backup which manages retention automatically

**4. Public IP addresses cost money**
- Static public IP: $3.65/month (even if not attached to anything)
- When you delete a VM or load balancer, the public IP often survives as an orphaned resource
- **Fix**: `az network public-ip list --query "[?ipConfiguration==null].{Name:name, RG:resourceGroup}"` to find unattached IPs

**5. Load balancers without backends**
- Standard Load Balancer: $18/month base + $5/rule, even with zero backends
- Application Gateway: $125+/month even with no traffic
- **Fix**: Delete unused load balancers. If you need them temporarily, recreate from Bicep templates -- infrastructure as code makes this trivial

**6. Database backups beyond retention**
- Automatic backups for Azure SQL: Included for up to 7-35 days (depending on tier)
- Long-term retention (LTR) backups: $0.25/GB/month -- a 500 GB database with weekly LTR backups for a year costs $6,500/year just for backups
- **Fix**: Set LTR policies that match your actual compliance requirements, not "just in case"

**7. Log Analytics ingestion (the silent killer)**
- Default log collection settings can ingest 50-500 GB/day for a medium application
- At $2.76/GB, that is $138-$1,380/day -- or $4,140-$41,400/month
- **Fix**: Enable sampling, exclude verbose diagnostic logs, and set daily caps. A 5 GB/day cap ($13.80/day) is sufficient for most applications

---

**Real-World Analogy: Your Electricity Bill**

Cost optimization in the cloud is exactly like managing your home electricity bill. You would not leave every light on in your house 24/7 "just in case" someone walks into a room. You turn off lights when you leave, use energy-efficient bulbs, and set the thermostat to a reasonable temperature. Cloud cost optimization follows the same principles: turn off what you are not using (deallocate VMs at night), right-size for actual need (do not rent a mansion for a studio apartment workload), and buy in bulk when you know you will use it (reserved instances). The difference is that a wasted light costs pennies -- a wasted cloud VM costs hundreds of dollars per month.

**What is Cost Optimization?** Cost optimization is the practice of reducing cloud spending while maintaining or improving performance. It's not about being cheap -- it is about spending wisely and eliminating waste.
...
<AccordionGroup>
  ...
  <Accordion title="3. Stop Dev/Test VMs" icon="power-off">
    ```bash
    # Enable auto-shutdown directly on the VM -- this is the simplest approach
    # and does not require Azure Automation. Available in Portal under
    # VM > Operations > Auto-shutdown, or via CLI:
    az vm auto-shutdown \
      --resource-group rg-dev \
      --name vm-dev-01 \
      --time 1800 \
      --timezone "Eastern Standard Time"
    # This deallocates the VM at 6 PM EST every day.
    # You still need to manually start it, or use Azure Automation for start.

    # For automated start + stop schedules across many VMs, use the
    # Azure Start/Stop VMs v2 solution (free, built into Azure Automation):
    # 1. Create Automation Account
    # 2. Enable "Start/Stop VMs during off-hours" feature
    # 3. Configure: Stop at 6 PM, Start at 8 AM, weekdays only
    # Savings: 60% (14 hours/day off + all weekends = 128 hrs off / 168 hrs/week)
    ```
  </Accordion>
