...
# Why both push and pull_request? Push triggers deploy to staging/prod.
# Pull_request triggers build+test only (validates the PR before merge).
on:
  push:
    branches:
      - main
...
jobs:
  build-and-test:
    # ubuntu-latest is the cheapest runner (~$0.008/min for private repos).
    # Only use windows-latest ($0.016/min) or macos-latest ($0.08/min)
    # if your app specifically requires those operating systems.
    ...
    steps:
      ...
      - name: Setup Node.js
        ...
        with:
          node-version: '18'
          # Enabling cache here saves 30-60 seconds per build by reusing
          # previously downloaded npm packages. Over hundreds of builds
          # per month, this saves significant CI minutes (and money).
          cache: 'npm'
      
      # Step 3: Install dependencies
      # Use 'npm ci' instead of 'npm install' in CI pipelines.
      # npm ci is faster (skips dependency resolution), more reliable
      # (uses exact versions from package-lock.json), and fails if
      # package-lock.json is out of sync -- catching a common dev mistake.
