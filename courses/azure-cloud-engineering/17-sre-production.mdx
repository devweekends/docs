...
<Tabs>
  <Tab title="Latency">
    **How long does a request take?**
    ...
    // Filter to successful requests only -- failed requests (5xx) often
    // have very short durations (fast error responses), which would
    // artificially lower your latency numbers and hide real problems.
    requests
    | where timestamp > ago(24h)
    | where success == true
    | summarize
        p50 = percentile(duration, 50),  // Median: "typical" user experience
        p95 = percentile(duration, 95),  // Slow users: 1 in 20 see this or worse
        p99 = percentile(duration, 99)   // Tail latency: 1 in 100 -- often reveals DB issues
        by bin(timestamp, 5m), name      // 5-minute buckets smooth out noise
    | render timechart
    ```
...
<div className="flex justify-center my-4">
  <div className="w-full max-w-4xl">
    ...
    graph TD
        ...
        B -->|> 50%| C[✅ Ship Features<br/>Take Risks<br/>Accelerate]
        B -->|10-50%| D[⚠️  Caution Mode<br/>Increase Testing<br/>Slow Down]
        B -->|< 10%| E[🚨 Stop Releases<br/>Fix Reliability<br/>Pay Down Debt]
...
<AccordionGroup>
  ...
  <Accordion title="3. CPU Stress" icon="microchip">
    ...
    **Expected**: HPA scales pods from 3 → 10 within 2 minutes
    **Actual**: ?
