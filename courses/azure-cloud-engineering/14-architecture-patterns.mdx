...
**Lesson**: Start simple. Evolve architecture as you grow. The companies that succeeded (Netflix, Amazon, Uber) all started with monoliths and migrated to microservices only after they had proven product-market fit, real scaling bottlenecks, and engineering teams large enough to justify the operational overhead.
...
│     Azure Service: App Service Basic tier or single B2s VM
│
├─ 100-10,000 users
│  └─ Answer: N-Tier Architecture ($500/month) ✅
│     Azure Services: App Service Standard + Azure SQL + Redis
│
├─ 10,000-1,000,000 users
│  └─ Answer: Microservices ($5,000/month) ✅
│     Azure Services: AKS + Cosmos DB + Service Bus + Front Door
│
└─ 1,000,000+ users
   └─ Answer: Multi-Region ($50,000/month) ✅
   Azure Services: Multi-region AKS + Cosmos DB multi-master + Front Door

Reminder: You can ALWAYS migrate later!
- Start simple → Evolve as you grow
- Don't build for 1M users when you have 100
- The cost of premature complexity is both financial (infrastructure)
  and organizational (slower iteration, harder debugging)
```
...
**Patterns**:
- **API Gateway**: Single entry point (Azure API Management). All external traffic enters through one door -- this gives you centralized authentication, rate limiting, and API versioning. Cost: ~$150/month for Developer tier, ~$2,800/month for Standard tier.
