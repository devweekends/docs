...
server {
    ...
    # Cache static assets aggressively. Angular's production build appends
    # content hashes to filenames (main.a1b2c3d4.js), so a new deployment
    # creates new filenames. This means it is safe to cache for 1 year --
    # the old cached files will simply never be requested again.
    location ~* \.(js|css|png|jpg|jpeg|gif|ico|svg|woff|woff2)$ {
        expires 1y;
        add_header Cache-Control "public, immutable";
    }
    
    # NEVER cache index.html. This is the entry point that references
    # the hashed JS/CSS files. If a user gets a stale index.html,
    # they will try to load old chunk files that no longer exist on the server.
    location = /index.html {
        expires -1;
