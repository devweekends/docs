...
```yaml
# Why: Prevents a developer from managing production VMs from a coffee shop WiFi.
# Even if their credentials are valid, requests outside the corporate network are denied.
# AWS equivalent: IAM policy with aws:SourceIp condition key.
- members:
...
```yaml
# Why: Developers can clean up their own temporary resources without needing admin help,
# but cannot delete anything outside the "temp-" prefix. This balances autonomy with safety.
# Common Mistake: Using resource.name conditions without testing -- the name format varies
# by resource type (some include project ID, some don't). Always test with Policy Troubleshooter.
- members:
