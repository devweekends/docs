...
**The key difference from AWS/Azure:** In AWS, an Application Load Balancer (ALB) is a regional resource -- you need one per region and must use Route 53 or Global Accelerator to stitch them together for global traffic. In GCP, a single Global HTTP(S) Load Balancer with one Anycast IP automatically routes users to the nearest healthy backend across all regions. This is not just a convenience; it eliminates the operational complexity of managing multiple regional load balancers and the DNS-based failover that comes with them.
...
A **Cache Key** is the string used to identify a cached object. By default, it's the full URL. Think of a cache key like a library card catalog entry -- if two requests generate the same "catalog entry," they get the same cached response.
- **Optimization**: You can include or exclude query strings, protocols, or specific headers from the cache key to improve the hit ratio. For example, if `?utm_source=twitter` should return the same content as `?utm_source=google`, exclude `utm_source` from the cache key.
- **Invalidation**: Propagates globally in minutes. You can invalidate by path (`/images/*`) or by header.

**Cost Tip:** Cloud CDN charges $0.02-$0.08/GB for cache egress (depending on region), compared to $0.08-$0.23/GB for origin egress. A well-tuned CDN with a 90%+ hit ratio can cut your egress bill by 60-75%. Monitor your cache hit ratio in Cloud Monitoring -- if it is below 80%, revisit your cache key configuration and `Cache-Control` headers.
...
1.  **Check Health Checks:** 90% of "LB not working" issues are due to failing health checks. Ensure your firewall allows the GCP health check ranges (`35.191.0.0/16` and `130.211.0.0/22`). This is the single most common GCP load balancing mistake -- it trips up beginners and experienced engineers alike.
2.  **Verify Firewall Rules:** Your backend VMs must have an ingress rule allowing traffic from the LB proxies on the service port. Remember, the LB does not use the same source IP as the client -- the GFE proxies the request, so your firewall must allow GFE source ranges, not just "the internet."
3.  **Inspect URL Map:** Use the console's "Traffic Distribution" view to ensure your path rules are actually routing to the correct backend services. A misplaced wildcard (e.g., `/*` before `/api/*`) can silently swallow all traffic.
...
**Practical Debugging Tip:** If your backends show healthy in the console but users see 502 errors, check if your application is binding to `0.0.0.0` (all interfaces) rather than `127.0.0.1` (localhost only). The health check probes come from the GFE, not from localhost, so a service bound to localhost will always fail health checks.
