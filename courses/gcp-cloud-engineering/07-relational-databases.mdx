...
Cloud SQL is a fully managed service for **MySQL, PostgreSQL, and SQL Server**. It is the go-to choice for migrating existing applications to the cloud or starting new projects that don't require global scale. If you are coming from AWS, Cloud SQL is the equivalent of Amazon RDS. If you are coming from Azure, it maps to Azure Database for MySQL/PostgreSQL/SQL Server. The feature set is comparable, but there are important differences in HA architecture and pricing that we will cover below.
...
gcloud sql instances create prod-db \
    ...
    --tier=db-custom-4-15360 \       # Why custom: 4 vCPUs, 15GB RAM -- tailored to our workload, avoids paying for unused RAM
    --region=us-central1 \
    --availability-type=REGIONAL \    # Why REGIONAL: Enables cross-zone HA with automatic failover (~60s)
    --storage-type=SSD \              # Why SSD: 30x IOPS vs HDD; use HDD only for dev/test to save ~40%
    --storage-size=100GB \
    --storage-auto-increase \         # Why: Prevents out-of-disk crashes. Storage only grows, never shrinks.
    --backup-start-time=03:00         # Why 03:00: Schedule during lowest-traffic window to minimize performance impact
```

**Cost Consideration:** HA doubles your compute cost (you pay for both the primary and standby instances). For a `db-custom-4-15360` in `us-central1`, that is roughly $0.54/hour ($390/month) for HA vs $0.27/hour ($195/month) for a single-zone instance. For production databases where downtime costs more than $195/month in lost revenue, HA is always worth it. For dev/test, skip HA and save the money.

**Common Mistake:** Forgetting that `--storage-auto-increase` only grows storage -- it never shrinks. If your database temporarily balloons to 500GB during a data migration, you will continue paying for 500GB even after you clean up the data. Plan your migrations carefully, or create a new instance and migrate the trimmed data.
...
```bash
# Why Auth Proxy: It creates an encrypted tunnel authenticated via IAM -- no need to
# manage SSL certs, IP allowlists, or database passwords in config files.
# AWS equivalent: RDS Proxy (but that is a separate managed service with its own pricing).
# The Cloud SQL Auth Proxy is free -- you only pay for the Cloud SQL instance itself.
curl -o cloud_sql_proxy https://dl.google.com/cloudsql/cloud_sql_proxy.linux.amd64
chmod +x cloud_sql_proxy

# The proxy listens on localhost:5432 and forwards traffic over an encrypted channel
./cloud_sql_proxy -instances=PROJECT_ID:REGION:INSTANCE_NAME=tcp:5432

# Your app connects to localhost as if the database were local -- zero code changes needed
...
```yaml
# Why sidecar pattern: The proxy runs alongside your app in the same pod, so your app
# connects to localhost. This eliminates network hops and simplifies firewall rules.
# The proxy handles IAM auth + TLS automatically. No database password rotation needed.
# Common Mistake: Forgetting to set resource requests/limits on the proxy container.
# The proxy uses ~50MB RAM and minimal CPU, but without limits, Kubernetes cannot bin-pack efficiently.
apiVersion: apps/v1
kind: Deployment
metadata:
  name: my-app
  ...
  template:
    spec:
      ...
      - name: app
        ...
        env:
        - name: DB_HOST
          value: "127.0.0.1"   # App talks to the sidecar on localhost
        - name: DB_PORT
          value: "5432"
...
```bash
# Why /16 prefix: Cloud SQL instances are created in a Google-managed VPC and connected
# to yours via peering. A /16 gives 65K IPs -- generous, but Cloud SQL creates hidden
# resources (HA replicas, proxy VMs) that consume IPs. A /20 works for small deployments
# but can run out if you have many instances. /16 is the safe default.
# Common Mistake: Using a range that overlaps with your existing subnets. This silently
# breaks peering and causes "connection refused" errors that are painful to debug.
...
# This creates the VPC peering between your network and Google's service networking
...
```bash
# Why same-region replica: Offloads read traffic from the primary, reducing latency for
# read-heavy workloads. Replication lag is typically <1ms within the same region.
# Cost: A replica costs the same as a standalone instance of the same tier.
gcloud sql instances create prod-db-replica-1 \
    --master-instance-name=prod-db \
    --region=us-central1 \
    --tier=db-custom-2-7680       # Smaller tier is fine if replica handles lighter read load

# Why cross-region replica: Serves two purposes -- (1) low-latency reads for European users,
# (2) disaster recovery if the US region goes down. You can promote this to a standalone
# primary if needed, but promotion is a one-way operation (it breaks replication).
# Common Mistake: Making the DR replica too small. During a failover, it becomes the primary
# and must handle ALL traffic. Size it to match (or exceed) the primary tier.
gcloud sql instances create prod-db-replica-dr \
    --master-instance-name=prod-db \
    --region=europe-west1 \
    --tier=db-custom-4-15360      # Match primary tier for DR readiness
```
...
```bash
# Why Sunday 4 AM: Choose the lowest-traffic window for your timezone. Maintenance can
# cause a brief restart (60-90 seconds with HA). If you skip setting a window, Google
# picks one for you -- and it might be during your peak hours.
# Common Mistake: Setting the window to "never" (you cannot). All Cloud SQL instances
# receive mandatory patches. Plan for it rather than being surprised.
gcloud sql instances patch prod-db \
    --maintenance-window-day=SUN \
    --maintenance-window-hour=04 \
    --maintenance-release-channel=production   # "production" = stable, less frequent updates
```
...
# Why the Cloud SQL Python Connector: It handles IAM authentication and TLS automatically,
# eliminating the need to run the Auth Proxy as a separate process.
# This is the recommended approach for Python apps since 2023.
connector = Connector()

def getconn():
    conn = connector.connect(
        "PROJECT:REGION:INSTANCE",  # Connection name from Cloud Console
        "pg8000",                    # PostgreSQL driver
        user="postgres",
        password="password",         # In production: fetch from Secret Manager, never hardcode
        db="mydb"
    )
    return conn

# Why pool_size=10: Each connection holds a PostgreSQL backend process (~10MB RAM).
# Cloud SQL limits connections based on your instance tier (e.g., 100 for db-custom-4).
# Setting pool_size too high wastes DB resources; too low causes connection wait times.
# Why pool_pre_ping=True: Validates connections before use, preventing "connection reset" errors
# after idle periods or Cloud SQL maintenance events.
engine = create_engine(
    "postgresql+pg8000://",
    creator=getconn,
    pool_size=10,        # Active connections to maintain
    max_overflow=5,      # Extra connections allowed during traffic spikes (total max: 15)
    pool_pre_ping=True   # Detect and replace stale connections automatically
)
```
...
AlloyDB is a PostgreSQL-compatible, enterprise-grade database designed for the most demanding transactional and analytical workloads. Think of AlloyDB as "Cloud SQL Pro" -- it uses the same PostgreSQL wire protocol (so your app needs zero code changes), but under the hood it replaces the standard storage engine with a Google-designed, log-structured architecture that is up to 4x faster for transactions and 100x faster for analytical queries. AWS's closest equivalent is Amazon Aurora PostgreSQL, which also separates compute from storage. Azure does not have a direct competitor at this tier.
...
```bash
# Why AlloyDB over Cloud SQL: If your PostgreSQL workload needs both fast transactions
# AND analytical queries (dashboards, reporting) on the same data, AlloyDB eliminates
# the need for a separate data warehouse. The columnar engine runs analytics 100x faster.
# Cost: AlloyDB starts at ~$0.50/vCPU-hour (~$2,900/month for 8 vCPUs), roughly 2x Cloud SQL.
# The premium pays for itself if it eliminates a separate BigQuery ETL pipeline.
gcloud alloydb clusters create prod-alloydb-cluster \
    --region=us-central1 \
    --password=secure-password    # Store this in Secret Manager, not in scripts

# Why 4096 MB for columnar engine: This determines how much analytical data is kept in-memory.
# More memory = more data cached in columnar format = faster analytical queries.
# Start with 25-50% of your instance RAM and tune based on query performance.
...
```bash
# Why read pools instead of read replicas: AlloyDB read pool nodes share the same storage
# layer as the primary, so there is zero replication lag (unlike Cloud SQL replicas which
# use async replication). All 3 nodes see the exact same data at all times.
# Common Mistake: Over-provisioning CPU on read pool nodes. Start with fewer vCPUs and let
# monitoring guide your scaling decisions.
gcloud alloydb instances create prod-alloydb-read-pool \
    --cluster=prod-alloydb-cluster \
    --region=us-central1 \
    --instance-type=READ_POOL \
    --cpu-count=4 \
    --read-pool-node-count=3       # 3 nodes for HA; AlloyDB auto-distributes reads across them
```
...
Cloud Spanner is the world's first "NewSQL" database. It provides the **scalability of NoSQL** with the **consistency of SQL** and **ACID transactions across continents**. There is no direct equivalent on AWS or Azure -- the closest comparisons are Amazon Aurora (for scale) and CockroachDB or YugabyteDB (for global distribution), but neither matches Spanner's combination of hardware-backed TrueTime and Google's network.

Think of Spanner as a "database that cheats physics." Normal distributed databases face the CAP theorem and must sacrifice either consistency or availability during network partitions. Spanner uses atomic clocks and GPS receivers to synchronize time across the globe with such precision that it can guarantee both -- at the cost of a small (under 10ms) "commit wait" on every transaction.
