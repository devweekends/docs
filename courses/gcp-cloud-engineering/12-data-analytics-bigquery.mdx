...
BigQuery is a serverless, highly scalable data warehouse. If you are coming from AWS, BigQuery is the equivalent of Amazon Redshift Serverless + Athena combined -- it offers both the power of a dedicated data warehouse and the convenience of querying data in place without provisioning clusters. Azure's closest equivalent is Azure Synapse Analytics. However, BigQuery was serverless from day one (launched in 2010), while its competitors added serverless modes years later.

To understand its power, you must understand its three internal components:
...
### Partitioning
Partitioning divides a large table into smaller segments based on a column (usually a date or timestamp). Think of it like organizing a filing cabinet by month -- when someone asks for October's invoices, you open one drawer instead of searching through all of them.
- **Pruning:** When you query with a `WHERE date = '2023-10-01'`, BigQuery only scans that specific partition, not the whole table. On a 10TB table with 365 daily partitions, a single-day query scans ~27GB instead of 10TB -- a 370x reduction in data processed and cost.
- **Types:** Ingestion-time, Time-unit column, or Integer-range.

**Cost Impact:** At $5/TB scanned (on-demand pricing), scanning 10TB costs $50 per query. With daily partitioning and a date filter, the same query costs $0.14. For a team running 100 queries/day, that is $5,000/day vs $14/day. Partitioning is not optional for production tables -- it is the single most impactful cost optimization in BigQuery.
...
**Example Query:**
```sql
-- Why: Summarize millions of customer reviews without exporting data or writing Python
-- temperature=0.2: Low creativity for factual summaries (higher = more creative/variable)
-- max_output_tokens=100: Keep summaries concise to control both cost and latency
-- Cost warning: Each ML.GENERATE_TEXT call invokes Vertex AI pricing per token
SELECT
...
**Cost Warning:** BQML with LLM integration charges both BigQuery data scanning fees AND Vertex AI inference fees. Running Gemini Pro over 1 million rows with 200 tokens input each can cost $50-200+ depending on token length. Always test with a `LIMIT 100` first to estimate costs before running on your full dataset.
...
# Why partition by timestamp: Most analytical queries filter by date. Without partitioning,
# a query scanning a 10TB table costs $50 at $5/TB. With daily partitions and a date filter,
# the same query scans ~27GB and costs $0.14. That is a 370x cost reduction.
# Why cluster by user_id, event_type: Further narrows the scan within each partition.
# Clustering order matters -- put the most-filtered column first.
# Common Mistake: Partitioning by a column with too few distinct values (e.g., "country").
# BigQuery supports up to 4,000 partitions; use ingestion-time partitioning if your column
# does not have enough cardinality.
bq load \
    --source_format=CSV \
