...
GCS isn't just "hard drives in a rack." It sits on top of **Colossus**, Google’s cluster-level file system (the successor to GFS).
...
**Pro-Tip:** Archive class in GCP is cheaper than almost any other storage in the industry, yet it still provides instant access without a 3-5 hour "thaw" period. This is a major differentiator from AWS S3 Glacier, which requires a restore request that can take minutes (Expedited), hours (Standard), or up to 12 hours (Bulk) before you can access the data. In GCP, you just read the object -- the only penalty is a higher per-GB retrieval fee, not a waiting period.

**Cost Math:** GCS Archive costs approximately $0.0012/GB/month for storage, but $0.05/GB for retrieval. If you store 10TB and retrieve 100GB/month, that is $12/month storage + $5/month retrieval = $17/month total. On AWS S3 Glacier Deep Archive, the same storage is $0.00099/GB/month ($9.90) but retrieval adds $0.02/GB + a restore request fee + the wait time. The GCP approach trades slightly higher retrieval cost for the massive operational simplicity of instant access.
...
A common "hidden cost" in GCS is incomplete uploads. If an upload fails halfway, the chunks stay in your bucket and you are charged for them! This same problem exists on AWS S3 and has been the cause of unexplained storage cost spikes at many companies.
...
- **How to detect existing waste**: Run `gcloud storage ls --recursive gs://your-bucket/ | grep ".compose"` to find orphaned composite parts. For large buckets, query the Cloud Storage inventory report in BigQuery instead.

**Real-World Scenario:** A media company was paying $400/month in storage for a bucket they thought contained 2TB of video files. Investigation revealed 8TB of incomplete multipart upload fragments from a buggy video processing pipeline that crashed frequently. A single lifecycle rule eliminated the waste entirely.
...
# Why --uniform-bucket-level-access: Prevents per-object ACLs that can accidentally make files public
# Why --location=us: Multi-region gives you automatic replication across US data centers
gcloud storage buckets create gs://my-global-assets \
    --location=us \
    --uniform-bucket-level-access

# 2. Enable Object Versioning
# Why: Protects against accidental deletion and ransomware -- you can always recover the previous version
gcloud storage buckets update gs://my-global-assets --versioning

# 3. Create a Lifecycle Policy (lifecycle.json)
# Why: Without lifecycle rules, storage costs grow indefinitely as old data accumulates
...
# Why gcloud storage (not gsutil): The newer Go-based CLI is 2-3x faster for large transfers
gcloud storage cp -r ./local-folder gs://my-global-assets/
```

**Common Mistake:** Setting `numNewerVersions: 1` thinking it keeps only the latest version. It actually keeps the current (live) version plus 1 non-current version. If you want *only* the live version, set `numNewerVersions: 0` in your delete rule for non-current objects. Getting this wrong means either keeping too many versions (wasting money) or deleting too aggressively (losing your safety net).
