...
- **No Overcommit:** Unlike some cloud providers, GCP does not overcommit CPU resources on standard machine types. If you buy 4 vCPUs, you get 4 physical threads of execution. This is in contrast to some AWS instance families where "burstable" instances (T-series) share CPU credits. GCP's E2 family does use dynamic resource scheduling (a form of sharing), but N2 and above provide dedicated resources.
- **Live Migration:** This is GCE's "killer feature." When Google needs to perform hardware maintenance or update a host kernel, it moves your running VM to a new host **without a reboot** or noticeable downtime. AWS and Azure both support maintenance events, but they typically involve a brief reboot. GCP's Live Migration is more like moving a running app between laptops without closing it -- the user never notices.

**Practical Impact:** Live Migration means you can run stateful workloads (databases, in-memory caches) on GCE with higher confidence than on other clouds. You will not get the "scheduled maintenance -- please reboot within 7 days" notifications that are common on AWS EC2.
...
Spot VMs are excess Google capacity offered at a massive discount (60-91%). The concept is identical to AWS Spot Instances and Azure Spot VMs, but with one important difference: GCP Spot VM pricing is fixed (the discount is set when you create the VM), whereas AWS Spot pricing fluctuates based on demand.
...
**Real-World Cost Example:** An `n2-standard-8` VM in `us-central1` costs approximately $0.39/hour on-demand. As a Spot VM, it costs roughly $0.12/hour -- a 70% savings. For a batch processing pipeline that runs 100 such VMs for 10 hours per day, that is a savings of $270/day or roughly $8,100/month.
...
def handle_termination(sig, frame):
    ...
    # 1. Flush logs -- do not lose the last few seconds of work
    # 2. Upload current progress to GCS -- so the next Spot VM can resume where this one left off
    # 3. Gracefully shutdown -- close DB connections, release locks
    # Why: You have only 30 seconds. If your cleanup takes longer, the VM is force-killed.
    # Pro-tip: Keep checkpoint files small and write them to a nearby regional GCS bucket.
    sys.exit(0)

# Listen for the SIGTERM signal sent by GCE when it reclaims the Spot VM
# This is the same signal used on AWS Spot (via the metadata endpoint) and Azure Spot
signal.signal(signal.SIGTERM, handle_termination)
```

**Common Mistake:** Not testing the termination handler in your CI pipeline. Many teams write the handler but never verify it works under real conditions. Use `kill -SIGTERM <pid>` locally to simulate the preemption, and ensure your checkpoint/resume logic is idempotent.
...
For non-production environments, use Instance Schedules to automatically start and stop VMs. This is equivalent to AWS Instance Scheduler or Azure Auto-shutdown, but it is natively built into GCE without needing a Lambda function or automation account.
- **Scenario**: Turn off dev servers at 6:00 PM and start them at 8:00 AM on weekdays (10 hours on, 14 hours off).
- **Benefit**: Reduces costs by ~65% for dev/test environments. For a team running 20 `n2-standard-4` dev VMs, this saves approximately $2,800/month.

**Pro-Tip:** Combine Instance Schedules with labels (e.g., `schedule:business-hours`) so you can audit which VMs are covered and catch any that were missed. The most expensive VMs in any organization are the ones running 24/7 in non-production environments because nobody remembered to turn them off.
...
# Why: Never use the default compute SA -- it has Editor on the entire project.
# A compromised VM with the default SA gives attackers access to everything.
gcloud iam service-accounts create web-worker-sa

# 2. Create Instance Template with Shielded Features
# Why --shielded-secure-boot and --shielded-vtpm: Protects against rootkits and boot-level malware.
# Why --scopes=cloud-platform: Scopes are a legacy access control. With a custom SA, the actual
# permissions come from IAM roles on the SA. The scope just sets an upper bound.
# Common Mistake: Using --scopes to restrict access instead of IAM roles. Scopes are coarse-grained
# and cannot be changed after VM creation without recreating the template.
...
# Why 3 zones: If one zone fails, the MIG automatically rebalances VMs across the remaining 2.
# This is the minimum for production HA. AWS equivalent: Auto Scaling Group across 3 AZs.
...
# 4. Set Auto-healing
# Why --initial-delay=300: Gives VMs 5 minutes to boot and start serving before health checks
# begin. Without this delay, freshly created VMs fail the check and get deleted in a loop.
# Common Mistake: Setting initial-delay too low for apps with heavy startup (Java, .NET).
