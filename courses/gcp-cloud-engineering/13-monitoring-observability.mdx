...
Think of observability like the instrument panel in an aircraft. You would never fly a plane with just an "engine on/off" light -- you need altitude, speed, fuel pressure, engine temperature, and hundreds of other indicators. The same principle applies to cloud systems. If you are coming from AWS, GCP's observability suite maps roughly to CloudWatch (Monitoring) + CloudWatch Logs (Logging) + X-Ray (Trace), but with tighter integration and native BigQuery SQL support for log analysis.
...
- **Exclusion Filters:** Drop logs you don't need (to save money). This is critical -- Cloud Logging charges $0.50/GB after the first 50GB/month free tier. A busy GKE cluster can easily generate 100GB+/month of logs. Excluding verbose debug logs, health check logs, and known-noisy services can cut logging costs by 40-60%.
- **Log Sinks:** Export logs to:
    - **Cloud Storage:** For long-term, low-cost compliance (WORM support). Best for logs you rarely query but must retain for regulatory reasons.
    - **BigQuery:** For deep analytical processing. Best for logs you want to query with SQL (incident investigation, trend analysis).
    - **Pub/Sub:** For real-time processing by a Cloud Function or external tool (like Splunk or Datadog). Best for triggering alerts or feeding a SIEM.

**Cost Optimization Tip:** Create an exclusion filter for GKE health check logs (`resource.type="k8s_container" AND textPayload:"GET /healthz"`). These logs are high-volume and low-value -- they confirm your service is running, but that information is already captured by your uptime checks and health check metrics. Excluding them can save $50-200/month on a busy cluster.
...
# Why: Log-based metrics turn log patterns into time-series data you can alert on
# This avoids polling logs manually -- the metric updates in near real-time
...
# Pro-tip: Use Terraform's google_monitoring_alert_policy resource for reproducibility

# 3. Create a Log Sink to export all Audit Logs to BigQuery
# Why BigQuery for audit logs: SQL queries are far more powerful for forensic investigation
# than the Log Explorer's filter syntax. "Who deleted what, when?" becomes a simple SQL query.
# Cost: You pay for BigQuery storage (~$0.02/GB/month) but gain powerful querying capability
gcloud logging sinks create audit-logs-to-bq \
    bigquery.googleapis.com/projects/$PROJECT_ID/datasets/audit_logs \
    --log-filter='logName:"logs/cloudaudit.googleapis.com"'
```

**Common Mistake:** Creating the sink but forgetting to grant the sink's service account write access to the BigQuery dataset. The sink will silently fail, and you will not discover the missing audit logs until you need them for an incident investigation. Always verify the sink is working by checking `gcloud logging sinks describe audit-logs-to-bq` and confirming the `writerIdentity` has the `BigQuery Data Editor` role on the target dataset.
