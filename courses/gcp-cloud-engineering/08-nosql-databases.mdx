...
Bigtable is Google's flagship NoSQL database. It powers Google Search, Maps, and Gmail. It is a sparsely populated table that can scale to billions of rows and thousands of columns. If you are coming from AWS, Bigtable is conceptually similar to DynamoDB but with a critical difference: Bigtable is a **wide-column store** (like Apache HBase, which is actually API-compatible with Bigtable) whereas DynamoDB is a key-value/document store. Bigtable excels at sequential range scans over massive datasets (time-series, IoT, analytics), while DynamoDB is optimized for point lookups with its partition key model.

Think of Bigtable as a massive, sorted spreadsheet. Data is stored in rows sorted by a single key (the Row Key), and you can have millions of columns. The genius is that this sorted structure allows you to scan ranges of data (like "all sensor readings from sensor-42 between 9:00 AM and 10:00 AM") incredibly fast.
...
**Important: This is a one-time, irreversible choice per project.** You must pick one mode when you create your first Firestore database, and you cannot switch later without creating a new project. Choose carefully.
...
| **AWS Equivalent** | Similar to DynamoDB + AppSync | Similar to DynamoDB (server-side) |

**Cost Advantage:** Firestore scales to zero. If your app has no traffic at 3 AM, you pay nothing. Compare this to Bigtable (minimum ~$1,400/month) or Cloud SQL (always-running VM). For startups and variable-traffic workloads, Firestore's pay-per-operation model is hard to beat.
...
# Why SSD: SSD nodes deliver ~10,000 QPS each with sub-10ms latency; HDD delivers ~500 QPS at 50ms+
# Why 3 nodes: Minimum for production. Bigtable needs enough nodes to distribute tablets evenly.
# Cost: ~$0.65/hr per SSD node = ~$1,400/month for 3 nodes. This is a significant baseline cost.
...
# Why: Cross-region replication gives you both disaster recovery AND low-latency reads globally
# The replication is asynchronous (eventual consistency) unless you use single-cluster routing
...
# Why one column family to start: Keep it simple. Add families only when you need different GC policies
# or different access patterns for subsets of columns
gcloud bigtable tables create sensor-data --instance=prod-bigtable --column-families=metrics
```

**Cost Warning:** Bigtable has a significant minimum cost. Even a 3-node SSD cluster costs roughly $1,400/month *before* storage. Bigtable is designed for workloads processing at least 300 QPS sustained. If your throughput is lower, consider Firestore (which scales to zero) or Cloud SQL. Bigtable is the wrong choice for small datasets -- it is like renting a warehouse to store a suitcase.
