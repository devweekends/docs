...
- **Global HTTP(S) Load Balancer:** A single anycast IP (`34.x.x.x`) fronting the whole world. In AWS, achieving this requires ALB + Route 53 + Global Accelerator -- three services instead of one.
- **Cloud Armor:** WAF policies to block SQLi/XSS and an "Edge Security Policy" to block non-launch countries. Equivalent to AWS WAF + Shield Advanced.
- **Cloud CDN:** Enabled for the media bucket, using **Signed Cookies** for private content. Equivalent to CloudFront with signed cookies.
...
- **Cloud Spanner (Multi-region Instance):** Storing user profiles, followers, and the "Social Graph" with external consistency using **TrueTime**. There is no true equivalent on AWS or Azure -- DynamoDB Global Tables offer eventual consistency, and Aurora Global Database is limited to single-writer regions.
...
| :--- | :--- | :--- |
| Cloud Spanner (multi-region, 3 nodes) | ~$6,570 | The largest line item; consider starting with regional and upgrading |
| GKE Autopilot (3 regions) | ~$1,200-2,400 | Depends on pod count and resource requests |
| Cloud Storage (5TB dual-region) | ~$115 | Turbo Replication adds ~15% premium |
| Memorystore Redis (6GB, Standard) | ~$330 | Standard tier for HA |
| Global LB + Cloud Armor | ~$200-400 | Armor policies add ~$5/policy/month |
| **Total Baseline** | **~$8,500-10,000** | Before traffic-based charges (egress, CDN) |

**Common Mistake:** Deploying multi-region Spanner from day one when your user base is in a single region. Start with a regional Spanner instance (~$2,190/month for 3 nodes) and migrate to multi-region when you actually need global consistency. Spanner supports online migration between configurations.
...
```hcl
# Why map(object(...)): Allows us to iterate over regions with for_each, creating
# identical infrastructure in each region without duplicating code. This is the
# "DRY" (Don't Repeat Yourself) principle applied to multi-region IaC.
variable "regions" {
  type = map(object({
    cidr        = string    # The primary subnet for VMs and services
    gke_master  = string    # /28 required for GKE private master endpoint
    gke_pods    = string    # /16 gives ~65K pod IPs per region -- generous but safe
    gke_svcs    = string    # /20 gives ~4K service IPs -- sufficient for most apps
  }))
  default = {
    "us-central1" = {
      cidr       = "10.0.1.0/24"
      gke_master = "172.16.0.0/28"     # Non-overlapping /28 blocks for each region's master
      gke_pods   = "10.1.0.0/16"
      gke_svcs   = "10.2.0.0/20"
    },
    "europe-west1" = {
      cidr       = "10.0.2.0/24"
      gke_master = "172.16.0.16/28"    # Offset by /28 to avoid overlap with us-central1
      gke_pods   = "10.3.0.0/16"
      gke_svcs   = "10.4.0.0/20"
    },
    "asia-east1" = {
      cidr       = "10.0.3.0/24"
      gke_master = "172.16.0.32/28"    # Continue the /28 offset pattern
...
# Common Mistake: Using overlapping CIDR ranges between regions. GCP will reject the
# subnet creation, but the error message is not always obvious. Plan your IP space
# on paper first, and leave room for expansion (e.g., a future 4th region).
...
3.  Run **Artifact Analysis** vulnerability scan -- fail the build if any CRITICAL CVEs are found.
4.  Create a **Binary Authorization** attestation for images that pass scanning.
5.  Trigger **Cloud Deploy** to roll out the image to the `dev` GKE cluster, followed by a manual approval gate for `production`.

**Common Mistake:** Skipping the vulnerability scan step "to move faster." This creates a direct pipeline from developer laptop to production with no security gate. The scan adds 30-60 seconds to your build and can prevent deploying a container with a known remote code execution vulnerability.
...
- One major challenge you faced during deployment. (Hint: IP range planning, IAM permission troubleshooting, and health check configuration are the most common blockers for first-time builders.)
- How you optimized for cost. (Quantify it: "By using Spot VMs for the Dataflow pipeline and Cloud Run instead of GKE for the image resizer, we reduced estimated monthly costs from $X to $Y.")
- How you ensured high availability if an entire zone goes down. (Demonstrate this by actually simulating a zonal failure and showing the recovery.)

**Pro-Tip for the War Story:** Interviewers love specificity. "We used multiple zones" is a weak answer. "We deployed GKE with 3 zones in us-central1, configured PodDisruptionBudgets to ensure at least 2 replicas were always available, and tested by cordoning an entire node pool -- traffic shifted in under 30 seconds with zero user-visible errors" is a strong answer.
