...
**Why this matters in practice:** In AWS, if you want a VM in `us-east-1` to talk to a VM in `eu-west-1`, you need VPC Peering or a Transit Gateway between two separate VPCs. In GCP, both VMs can live in the *same* VPC with subnets in different regions, communicating over internal IPs without any peering setup. This is not a minor convenience -- it fundamentally simplifies global application architectures and eliminates an entire class of networking overhead.
...
```bash
# Why: VMs without public IPs still need to call Google APIs (GCS, BigQuery, etc.)
# Private Google Access routes this traffic over Google's backbone, never touching the internet
# AWS equivalent: VPC Gateway Endpoints for S3/DynamoDB, Interface Endpoints for other services
...
**Cost Consideration:** Private Google Access is free. However, if you skip it and instead route API traffic through a Cloud NAT to reach Google APIs over the internet, you will pay NAT processing fees and egress charges -- typically $0.045/GB. For a workload that transfers 1TB/month to Cloud Storage, that is $45/month you did not need to spend.
...
Shared VPC is the architectural "hub-and-spoke" model for Google Cloud. It allows a central network team to manage resources while allowing application teams to consume them. If you are coming from AWS, Shared VPC is conceptually similar to AWS Resource Access Manager (RAM) sharing subnets across accounts, combined with AWS Transit Gateway for centralized routing -- but it is a single, native feature rather than requiring multiple services.
...
Think of Cloud NAT as a one-way door for your private VMs -- they can go out to the internet (to download packages, call external APIs), but nobody from the outside can walk in. Unlike traditional NAT gateways (or AWS NAT Gateways) that are single VMs you must size and manage, Cloud NAT is a distributed, software-defined service built into the Andromeda SDN.
...
**Cost Comparison:** AWS NAT Gateway charges $0.045/GB processed + $0.045/hour. GCP Cloud NAT charges $0.044/GB processed + $0.0014/hour per allocated IP. For high-throughput workloads, GCP Cloud NAT is typically 20-30% cheaper than its AWS equivalent, and it does not have the throughput ceiling of a single AWS NAT Gateway instance.
