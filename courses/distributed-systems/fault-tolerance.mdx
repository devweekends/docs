...
The mental model shift required here is profound: on a single machine, you write code assuming success and handle failure as a special case. In distributed systems, you must design assuming failure is the default state and success is the special case that needs verification. A senior engineer does not ask "What if this call fails?" but rather "What happens when this call fails, because it will?"
...
Think of timeouts like a "dead man's switch" on a train. The conductor must press a button every few seconds to prove they are conscious and in control. If they stop pressing it (because they are asleep, incapacitated, or gone), the train stops automatically. Without this switch, a train with an incapacitated conductor keeps going indefinitely. Your service without timeouts is that train -- when a dependency goes unresponsive, your threads pile up waiting forever until the entire service collapses.
...
class Retry:
    ...
    Why jitter matters: Imagine 1,000 clients all timing out at t=0. Without
    jitter, they all retry at t=100ms, then t=200ms, then t=400ms -- creating
    synchronized "waves" of load that repeatedly overwhelm the server. With
    jitter, the first wave spreads from t=80ms to t=120ms, the next from
    t=160ms to t=240ms, and so on. The load becomes smooth rather than spiky.
    
    This is the difference between a crowd all pushing through a door at once
    versus filing through at staggered intervals.
    """
...
**The electrical analogy is exact**: A circuit breaker in your home detects overcurrent and "trips" (opens), cutting power to that circuit before it catches fire. When the problem is resolved, you manually reset the breaker. A software circuit breaker does the same thing: it detects too many failures to a dependency and "trips," immediately rejecting all requests to that dependency instead of waiting for them to time out. After a cooling period, it lets a few test requests through (half-open state). If those succeed, the circuit closes and normal traffic resumes.
...
class CircuitBreaker:
    ...
    Based on Netflix Hystrix patterns. Key design decisions:
    - Uses a sliding window (not a simple counter) to measure failure rate,
      so a burst of failures followed by successes will eventually close
      the circuit without manual intervention.
    - Half-open state allows a small number of "canary" requests through
      to test if the dependency has recovered, avoiding the thundering herd
      that would occur if all traffic resumed at once.
    - The failure_threshold is a RATE, not a count. This matters because
      10 failures out of 10 requests is very different from 10 failures
