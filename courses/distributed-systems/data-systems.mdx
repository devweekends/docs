...
class ConsistentHash:
    ...
    The ring is represented as a sorted list of hash values. Each physical 
    node gets many positions on the ring (virtual nodes) to ensure even 
    distribution. Looking up a key means hashing it and walking clockwise 
    to the first node position -- an O(log N) binary search.
    """
    def __init__(self, nodes=None, virtual_nodes=100):
        self.virtual_nodes = virtual_nodes  # More virtual nodes = better balance, more memory
        self.ring = {}  # hash_value -> physical_node_id
        self.sorted_hashes = []  # Sorted for binary search
    ...
    def _hash(self, key):
        """Generate hash for a key. MD5 is used here for uniform distribution,
        NOT for security. In production, consider xxHash or MurmurHash3 for speed."""
        return int(hashlib.md5(
            key.encode()
        ).hexdigest(), 16)
    
    def add_node(self, node):
        """Add a node with virtual nodes. Each virtual node is a separate
        position on the ring, so one physical node 'owns' many arcs."""
    ...
    def remove_node(self, node):
        """Remove a node and its virtual nodes. Only keys that were assigned 
        to this node need to be reassigned -- they move to the next node 
        clockwise on the ring. All other keys are unaffected."""
        for i in range(self.virtual_nodes):
            virtual_key = f"{node}:{i}"
            h = self._hash(virtual_key)
            del self.ring[h]
    ...
    def get_node(self, key):
        """Find the node responsible for this key by walking clockwise."""
        if not self.ring:
            return None
        
        h = self._hash(key)
        
        # Binary search: find the first node position >= key's hash
    ...
    def get_nodes(self, key, n=3):
        """Get n distinct physical nodes for replication. Walk clockwise and 
        skip virtual nodes belonging to the same physical node -- you do not 
        want all replicas on the same machine."""
        if not self.ring:
            return []
...
│                                                                              │
│  The key insight: CockroachDB proves you can build a globally               │
│  consistent database WITHOUT custom hardware. The trade-off is that         │
│  under clock skew, some transactions may need to restart (Spanner           │
│  avoids this with TrueTime's bounded uncertainty). In practice,             │
│  NTP skew in a well-configured cluster is small enough that                 │
│  restarts are rare, making this a practical engineering compromise.          │
│                                                                              │
│  KEY DIFFERENCES FROM SPANNER:                                              │
│  ─────────────────────────────                                              │
│  • No TrueTime → uses HLC (Hybrid Logical Clocks)                          │
│  • May require transaction restarts for clock skew (rare in practice)       │
│  • Uses Raft instead of Paxos (easier to understand and implement)          │
│                                                                              │
│  ARCHITECTURE:                                                              │
│  ─────────────                                                              │
│                                                                              │
