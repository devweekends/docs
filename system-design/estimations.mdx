...
Back-of-envelope estimation is the skill that separates engineers who *design* systems from engineers who *describe* systems. When you estimate that your Twitter-like system needs 300,000 QPS for timeline reads, that single number immediately tells you: a single database will not work, you need aggressive caching, and fan-out-on-write (pre-computing timelines) makes more sense than fan-out-on-read. The estimate drives the architecture.
