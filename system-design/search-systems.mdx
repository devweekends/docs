...
Think of a search system like the index at the back of a textbook. Instead of reading every page to find "machine learning," you flip to the index, find the term, and it tells you exactly which pages mention it. That is an inverted index -- the core data structure behind every search engine. The hard parts are not the lookup itself (that is fast), but everything around it: understanding what the user *meant* to search for (query understanding), deciding which of 10 million results is *most relevant* (ranking), and doing all of this in under 100ms.
...
## Trade-off Analysis: Search Architecture Decisions

| Decision | Option A | Option B | When to Choose A | When to Choose B |
|----------|----------|----------|------------------|------------------|
| Index update | Real-time | Batch | Fresh results critical (e-commerce inventory) | Freshness delay acceptable (blog search) |
| Relevance | BM25 (statistical) | ML re-ranking (L2R) | Small team, limited training data | Large catalog, user behavior data available |
| Sharding | By document | By time | General search (products, users) | Time-scoped queries (logs, events) |
| Typo handling | Fuzzy at query time | n-gram at index time | Low index storage budget | Fast query performance required |

<Tip>
**Practical tip -- a senior engineer would say:** "The biggest mistake teams make with Elasticsearch is treating it as a primary database. ES is an index, not a source of truth. Always have a separate source of truth (PostgreSQL, DynamoDB) and rebuild your ES index from it. When -- not if -- your ES cluster has issues, you need to be able to reindex from scratch. Design for this from day one."
</Tip>

<Warning>
**Cardinality trap**: High-cardinality fields (like user IDs or session tokens) in Elasticsearch aggregations can consume enormous heap memory and crash your cluster. A senior engineer would say: "Never aggregate on unbounded cardinality fields. Use composite aggregations with pagination, or pre-aggregate in your data pipeline before indexing."
</Warning>
...
5. **Operational maturity**: "We'd keep a separate source of truth and rebuild the index from it -- ES is a search index, not a database"
</Tip>

### Common Questions

**Q: How would you implement autocomplete?**
> Use a completion suggester or prefix trie. Store popular queries/titles. Rank by popularity/recency. Cache heavily as prefixes are repeated. Practical tip: the top 1000 prefixes cover 80%+ of autocomplete traffic -- cache them aggressively at the CDN layer.

**Q: How does Elasticsearch scale?**
> Sharding for write scaling (each shard is an independent Lucene index). Replication for read scaling. Queries scatter to shards, gather and merge results. Key trade-off: more shards means more parallelism but higher coordination overhead. A common mistake is over-sharding -- start with 1 shard per 30-50GB of data.

**Q: How would you handle typos in search?**
> Fuzzy matching with edit distance (Levenshtein). n-gram tokenization for partial matches. Spell correction using dictionary lookups. Trade-off: fuzzy matching at query time is simpler but slower; n-gram indexing is faster at query time but increases index size by 3-5x.
